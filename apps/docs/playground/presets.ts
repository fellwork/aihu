...
const ROUTE = `@state {
  ...
  // Interpolation must be a single binding, so derive the heading in @state.
...
@template {
  ...
  <section class="page">
    <h1>{heading}</h1>
    <p>You are viewing the <code>{page}</code> route. In a real app, @aihu/router maps file paths to routes.</p>
  </section>
}
