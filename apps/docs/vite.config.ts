  ...
  return {
    ...
    closeBundle: {
      ...
      handler() {
        ...
        if (!flatHtmlLogged) {
          flatHtmlLogged = true
          console.log(`[docs] flat-html: ${written} extensionless sibling(s) emitted`)
        }
      },
    },
  }
...
// removed framework-wide, so the docs app carries its own config here rather than
