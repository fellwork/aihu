 ...
 * from a page's `@state` block — docs-next is an `output: 'static'` app and
 ...
 * `apps/docs-next/scripts/stage-wasm.ts` → `scripts/build-wasm-bundle.ts`
 ...
 * the root. docs-next is a file-routed SSG app: `/playground` prerenders to
 ...
 * anchors — in docs-next the router owns `location.pathname` and the hash
...
export class PlaygroundEmbed extends HTMLElement {
  ...
  private async boot(): Promise<void> {
    ...
    if (mod === null) {
      this.setError(
        'WASM bundle unavailable. Install wasm-pack + the wasm32-unknown-unknown target and rebuild docs-next (bun run prebuild) to populate public/wasm/.',
      )
      return
    }
    this.wasm = mod
