...
**Written by:** Historian (v0.8 closeout: @aihu/cli scaffolder + Hello World template; 570 TS tests; Learning #39 added)
**Track:** plan-a (TypeScript runtime family — signals → arbor → runtime → agent)
**Active branch:** `claude/aihu-phase-3-team-Za4UQ`
**Develop-on branch (all sessions):** `claude/aihu-phase-3-team-Za4UQ`
...
| 4 | `@aihu/runtime` | ✅ Done — `defineElement` + `defineComponent` + `DefineOptions` + `ShadowMode` | team merge commit (Phase 4 PASS first try) |
| 5 | `@aihu/agent` | ✅ Done — `getAgentMetadata` + `registerAgentMetadata` + `AgentMetadata` registry | team merge commit (Phase 5 PASS WITH NOTES, 3 LOW all ACCEPT) |
| N+1 | `bench/arbor` | ✅ Done — 6×6×2 SOTA harness; aihu fastest on mount + update vs JSDOM-compatible comparators | team branch merge |
| N+1 | `bench/signals` | ✅ Done — memory runner + 3 parity workloads; aihu wins cellx+dynamic-deps, loses deep-propagation | team branch merge |

**Final v0 footprint (all 4 packages, gz, post-Round-N):**

| Package | Size | Budget | Headroom |
|---|---|---|---|
| `@aihu/signals` | 1.53 kB | 1.7 kB | 172 B (10%) |
| `@aihu/arbor` | 1.28 kB | 2.05 kB | 772 B (38%) |
| `@aihu/runtime` | 438 B | 1.02 kB | 586 B (57%) |
| `@aihu/agent` | 72 B | 100 B | 28 B (28%) |
...
| Plan staleness banner (Option 1) | LOW | Doc-only, 10 min. Update `docs/superpowers/plans/2026-04-24-aihu-v0-plan-a-ts-runtime.md` to acknowledge Phases 4+5 shipped. |
...
1. bench/arbor/ — 6 workloads × 6 competitors × 2 metrics (time + memory). Aihu fastest on all mount + update workloads vs JSDOM-compatible comparators.
2. bench/signals/ — memory runner + 3 parity workloads (deep-propagation-100, dynamic-deps, creation-1to1000). Aihu wins on cellx, batched-writes, dynamic-deps, creation; loses deep-propagation to alien-signals (1.65×, documented as design-point gap).
...
| Architect R2.1 (v0.2→1.0 roadmap) | `.team/v1-reconciliation/roadmap-v1.md` → `docs/superpowers/plans/2026-05-02-aihu-v1-framework.md` | moved at Builder R4 |
...
**Key findings:**
- Spec quartet ~62/95 GAP from current aihu code; full syntax migration is a redesign, not extension
- v3 dep-free thesis (Learning #49) audit: aihu is essentially compliant at runtime today; only HMR client and select build-time tooling remain on Vite
- Router middleware Option 1 (isomorphic) keeps client-side nav guards inside `@aihu/*` (Option 2 would have pushed consumers to npm router-middleware libs)
...
- Build (per-package `bun run --filter '@aihu/*' build`): all 8 packages PASS; `bun run build` aggregate fails on pre-existing `baselines:build` (no `rolldown.config` — known, non-blocking).
...
Plan at `docs/superpowers/plans/2026-05-02-aihu-v1-framework.md`. Spec
...
- Architect R2.1 source draft: migrated (was `.team/v1-reconciliation/roadmap-v1.md`; now `docs/superpowers/plans/2026-05-02-aihu-v1-framework.md`)
...
|----------|-----|--------|--------|-------|
| v0.2.1 | #29 | feat/v0.2.1-plugin-entry-point | `8c76b41` | @aihu/plugin package + defineAihuConfig.plugins field; +12 tests |
...
| v0.2.6 | #33 | feat/v0.2.6-data-plugin-registration | `8c4d33e` | @aihu/data plugin shim; +3 tests; NOTE: @aihu/data limit raised 750→800 B for plugin shim bytes |
...
| `@aihu/router` | 1.45 kB | 1536 B | +50 B |
...
### Notable finding: @aihu/data limit raise (v0.2.6)

The plugin registration shim added 28 B gz to the `@aihu/data` bundle even when using `import type` + a direct object literal. The v0.2.6 Builder raised the `@aihu/data` limit from 750→800 B. Per the framework plan §"What this roadmap does NOT do" item 4 ("Re-opening any v0 size budget"), this was NOT pre-authorized — however, per Learning #42 split rationale, these are FEATURE bytes (new plugin registration capability), not debt bytes. The Director note's assumption that the shim would be zero-runtime-bytes was incorrect. Documented as Learning #37.
...
| v0.3 fixtures | #35 | feat/v0.3-block-grammar-migration | `4768672` | 5 `.aihu` + 5 `.golden.js` in `bench/compiler-conformance/blocks/` |
...
|----------|-----|--------|-------|
| v0.4.9 | #36 | A (TS) | `onMount` + `onCleanup` in `@aihu/runtime`; Compressor pass (213 B raw recovered) |
...
| v0.6.3 | #41 | TS | Router vite-plugin reads .route.json; `@aihu/router/plugin` subpath |
| v0.6.5 | #41 | TS | `BuildTarget` + `AihuConfig.build?` in @aihu/server |
| v0.6.7 | #41 | TS | `createServerCall<Args,Return>` in @aihu/server/client.ts |
| v0.6.8 | #41 | TS | `virtual:aihu-layouts` + `scanLayouts()` in @aihu/router/plugin |
...
| `@aihu/agent` | 117 B | 200 B | +83 B |
| `@aihu/data` | 778 B | 800 B | +22 B |
| `@aihu/router` | **740 B** | 1536 B | **+796 B** (dropped 711 B via browser/plugin split) |
| `@aihu/agent-service` | 580 B | 600 B | +20 B |

**Notable:** `@aihu/router` browser bundle dropped from 1451 B to 740 B after v0.6b separated build-time vite-plugin code into `@aihu/router/plugin` subpath. The +256 B router limit raise pre-authorized for v0.7.1 is now likely unnecessary.
...
| v0.7.3 | #42 | `Plugin.serverOnly?: boolean` added to @aihu/plugin |
...
v0.8 = CLI scaffolder + Hello World template + first-run UX (`@aihu/cli`).
...
**Date:** 2026-05-03
**Written by:** Historian (v0.8 closeout: @aihu/cli scaffolder shipped; 570 TS tests; Learning #39 added)
...
| Sub-item | PR | Notes |
|----------|----|-------|
| v0.8.1 | #43 | `@aihu/cli` package — `aihu app`, `aihu page`, `aihu component`, `aihu plugin`, `aihu migrate` commands; stdlib-only, zero deps |
| v0.8.2 | #43 | Hello World template — `package.json`, `aihu.config.ts`, `vite.config.ts`, `src/pages/index.aihu`, `src/layouts/default.aihu` |
| v0.8.5 | #43 | Plugin scaffold template — `aihu-plugin-<name>/package.json` + `src/index.ts` |
| docs | #43 | `docs/cli.md` light-off procedure |
| fix | `90c95dc` | `@aihu/cli` classified `BUILD_DEV_ONLY` in `check-size-rows` (post-merge fix) |
...
**check-size-rows omission:** `@aihu/cli` was not classified in `check-size-rows` in the initial PR. Fixed post-merge at `90c95dc`. Learning #39 adds "check-size-rows classification" as a mandatory step on the package-creation checklist.
...
| v0.9.4 | #44 | `scripts/hmr-check.ts` — HMR aihu-native confirmation; PASS |
...
**Dep-free thesis confirmed.** All 12 packages pass `dep-check.ts` — zero non-`@aihu/*` runtime dependencies across the workspace.

### Durable references (v0.9)

- Retro: `.team/v0.9/retro.md`
- Director note: `.team/v0.9/director-note-session-start.md`
- Next: v1.0 cutover — see `docs/superpowers/plans/2026-05-02-aihu-v1-framework.md` §v1.0
