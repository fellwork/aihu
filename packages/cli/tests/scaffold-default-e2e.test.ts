...
describe.skipIf(!RUN_E2E)('default scaffold e2e', () => {
  ...
  it('emits a buildable project: install + typecheck + build all succeed', () => {
    ...
    //
    // Asserting on file CONTENT, not just presence. FEL-423's whole failure
    // mode was documents that existed, returned 200, and said nothing — an
    // `existsSync` check passes on every one of them.
    ...
    //
    // This file is currently ~60 bytes: a title and a summary, with no
    // `## Components` section, even though src/pages/index.aihu declares three
    // `$action` entries. Cause is in the compiler, not here: `elide_agent`
    // (codegen/emit.rs) strips `registerAgentMetadata(...)` from CLIENT-target
    // builds entirely, so nothing populates the registry the generator reads.
    //
    // There is deliberately NO `expect(llms).not.toContain('## Components')`
    // here. Asserting the absence would certify the defect as correct and go
    // green forever — which is exactly how the existing plugin unit test
    // ("omits the Components section when the registry is empty") let this
    // ship. When the compiler emission lands, ADD the positive assertion.

    const robots = readFileSync(dist('robots.txt'), 'utf8')
    expect(robots).toContain('User-agent:')

    // The MCP server card must NOT be emitted. This scaffold declares no
    // `endpoint` because a static client build has no process to serve one;
    // publishing a card would advertise a transport that answers nothing.
    // The previous scaffold set `endpoint` to the card's OWN url and shipped
    // a card listing zero tools (FEL-423).
    expect(
