...
Platform-specific native binary distributor for `@aihu/server`
...
| Triple | npm package |
|---|---|
| `darwin-arm64` (Apple Silicon) | `@aihu/server-darwin-arm64` |
| `darwin-x64` (Intel macOS) | `@aihu/server-darwin-x64` |
| `linux-x64-gnu` (glibc Linux) | `@aihu/server-linux-x64-gnu` |
| `win32-x64-msvc` (Windows) | `@aihu/server-win32-x64-msvc` |

If you genuinely need to install only this package (e.g. for an offline mirror):

```bash
npm install @aihu/server-darwin-arm64
# or
bun add @aihu/server-darwin-arm64
```

<sub><i>Auto-generated against `@aihu/server-darwin-arm64@0.1.0` on commit `fead070`.</i></sub>
...
|---|---|
| **Version** | `0.1.0` |
| **Tier** | platform — Platform-specific native binary distributor for `@aihu/server` |
