//! `palette.rs` — Tailwind v4 default color palette (oklch), generated from
//! `tailwindcss@4.2.4/theme.css`. Palette utilities (`bg-amber-500`,
//! `from-red-200`, …) compile to `var(--color-<family>-<shade>)`; the scoped
//! emitter registers the ones a component references so they resolve at
//! `:host` — matching Tailwind, which ships the full palette in its default
//! theme. 286 entries.

/// The Tailwind v4 oklch value for a `<family>-<shade>` palette token
/// (`"amber-500"` → `"oklch(76.9% 0.188 70.08)"`), or `None` if not a known
/// palette token.
pub fn oklch(token: &str) -> Option<&'static str> {
    Some(match token {
