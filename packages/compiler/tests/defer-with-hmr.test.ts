...
const HMR_PROCESSED = `import { branch, leaf } from '@aihu/arbor'
import { signal } from '@aihu/signals'
import { defineComponent, defineElement, _hmrReplace } from '@aihu/runtime'
let __aihu_setup__: ((ctx: any) => any) | undefined
defineElement('x-counter', defineComponent(__aihu_setup__ = (_ctx) => {
  const [c] = signal(0)
  return branch('span')
}))
...
describe('_buildDeferredHydration after HMR pass', () => {
  it('closes defineElement BEFORE the export postamble (#1)', () => {
    ...
    expect(out).toMatch(
      /defineElement\(\s*'x-counter'\s*,\s*__aihu_wrap_defer__\(\s*defineComponent\([\s\S]*?\)\)\)\nexport\s+\{\s*__aihu_setup__/,
    )
  })
})
