...
fn compile_impl(source: &str, expr_parser: crate::ExprParserMode) -> Result<JsValue, JsValue> {
    ...
    // references and the manifest, then enforce C450. This entry point always
    // emits JS (the playground compiles to a runnable module), so the define
    // name always reaches `customElements.define` and the rule always applies.
    let tag_name = crate::envelope::validate_define_tag(&tag_name)
        .map_err(|e| JsValue::from_str(&crate::envelope::format_compile_error(&e)))?;
