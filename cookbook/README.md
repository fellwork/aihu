...
| File | Description |
|---|---|
| `counter.aihu` | `$prop count` + `$action increment/decrement/reset` — minimal reactive counter |
...
| `controller.aihu` | `$controller` with a `ResizeObserver` — `mount`/`disconnect` lifecycle hooks |
...
| `clock.aihu` | `$lifecycle.mount` starts `setInterval`; `dispose` clears it — real-time clock |
| `accordion.aihu` | `$prop items` + `$action toggle`; `{#each}` + `$show` per panel |
| `tabs.aihu` | `$prop tabs` + `$action selectTab`; `$computed selected` drives active panel |
| `modal.aihu` | `$prop open`, `$action show/hide`, `$on.keydown` for Escape, `$aria` role=dialog |
| `toast.aihu` | `$prop message`, `$action dismiss`, auto-dismiss via `$lifecycle.mount` setTimeout |
