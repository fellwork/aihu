/**
 * `@scribe/arbor` adapter for the bench harness.
 *
...
import { mount as arborMount, type MountScope, type Node } from '@scribe/arbor'

import type { AdapterContext, DomAdapter } from '../types.ts'

/**
 * The scribe adapter expects each workload to provide a `buildTree()` factory
 ...
 * `setupScribe(host, () => tree)` directly rather than going through the
 ...
 * vs scribe Branch/Leaf).
 */
export interface ScribeWorkloadHook {
  buildTree(): Node
  /** Called once mount completes; receives the mount scope so the workload can drive updates. */
  attach?(scope: MountScope): void
}

let pendingHook: ScribeWorkloadHook | null = null

/** Workload-side helper: register the tree builder before calling adapter.setup(). */
export function setScribeHook(hook: ScribeWorkloadHook): void {
  pendingHook = hook
}

export const scribe: DomAdapter = {
  name: '@scribe/arbor',
  version: 'workspace',
  setup(host: Element) {
    const hook = pendingHook
    if (!hook) {
      throw new Error('scribe adapter: workload must call setScribeHook(hook) before setup()')
    }
    pendingHook = null
