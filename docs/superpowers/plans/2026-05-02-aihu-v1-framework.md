# aihu v0.2 → v1.0 Framework Roadmap
...
> - **v0.8:** `npx aihu migrate` auto-converter ships in `@aihu/cli` to convert v0.1.x SFCs in-place.
...
## Current state baseline (aihu v0.1.x at main `7fa0957` + this session's outputs)

**Tests:** 454/454 passing across 54 files. **Sizes:** 7 of 8 packages within budget; `@aihu/arbor` is 15 B OVER (regression carried forward from R1 Scout).

| Package | Size | Limit | Headroom | Status |
|---|---|---|---|---|
| `@aihu/context` | 249 B | 300 B | +51 B | shipped (v0.1) |
| `@aihu/signals` | 1.81 kB | 1970 B | +120 B | shipped (v0; frozen) |
| `@aihu/arbor` | 2.16 kB | 2200 B | **−15 B OVER** | shipped (v0.1) — regression |
| `@aihu/runtime` | 1.14 kB | 1170 B | +7 B | shipped (v0.1) |
| `@aihu/agent` | 117 B | 200 B | +83 B | shipped (v0; frozen) |
| `@aihu/data` | 711 B | 750 B | +39 B | shipped (v0.1) |
| `@aihu/router` | 1.45 kB | 1536 B | +50 B | shipped (v0.1) |
| `@aihu/agent-service` | 580 B | 600 B | +20 B | shipped (v0.1) |
| `@aihu/server` | (no row) | n/a | n/a | shipped (v0.1) — see v0.2 row policy |
| `@aihu/agent-readiness` | (no row) | n/a | n/a | shipped (v0.1) |
| `@aihu/compiler` | (Rust) | n/a | n/a | Phase 1 + C-3/C-4 shipped; HTML-tag grammar |

**Spec-quartet alignment headline (from Scout R2.5):** SHIPPED ~6, PARTIAL ~18, GAP ~62, UNKNOWN ~5. The v0.1.x packages ship the underlying *primitives* (`when`, `each`, `slot`, `signal`, `effect`, `createResource`, `defineMiddleware`, `composeMiddleware`, `defineLoader`, `mountResource`, `_hmrReplace`, file-based router) — what's missing is the **binding contract**: the `@blockname { }` block grammar, the `$attr` template syntax, the `<$element>` macro elements, the `@route` block, the build target framework, the `$server` split-bundle artifacts, the `@aihu/plugin` contract surface itself.

**Runtime dep envelope:** zero non-`@aihu/*` runtime deps across all packages on `main`. Already substantially v3-thesis-compliant at runtime; the roadmap is to keep it that way through v1.0.
...
- **Template attribute syntax** (Template Attribute Syntax Spec §§1-6): two forms only — quoted (`attr="value"` for identifier ref / dotted property path / iteration token) and curly (`attr={expression}` for arbitrary JS). Bare values forbidden. Boolean-only attributes (HTML `disabled`, aihu `$once`, `$raw`). Per-macro type matrix (§3.3) is the source of truth. Slot/fallback hybrid for `<$suspense>`/`<$shield>`/`<$guard>`. Inline JSX in attributes forbidden in v1.
...
- **Q3:A** — file-based layouts (`layouts/default.aihu`, Nuxt-style). Locked. Pages override via `@route { layout: 'admin' }`; `@layout 'admin'` shorthand exists per spec §7.5 (kept for v1.0; Open Q in spec §12 to drop it is deferred to v1.1).
- **Q6 router middleware: Option 1** — router-level isomorphic `defineRouterMiddleware`, distinct from server `defineMiddleware`, composable across the SSR boundary at `composeMiddleware(allMws)(req, finalHandler)`. +150-220 B in `@aihu/router`; raises router size limit by +256 B in v0.7.
...
- **Path convention reconciliation**: Scout R2.5 was written against Option A (`_aihu-server/{actions,form-actions,mcp}/`). User locked Option B (`/server/_actions/`, `/server/_form-actions/`, `/server/_mcp/`) for this session. The spec text needs an inline reconciliation when the spec migrates to `docs/superpowers/specs/`; Builder R4 owns that diff. **Adaptation feasible — single-line path-prefix change in spec text.**
...
- **`createRouter`** exists as an export of both `@aihu/server` (request-router for HTTP envelope) and `@aihu/router` (file-based route table consumer). Same identifier, different intent.
- **`defineMiddleware`** in `@aihu/server` (hand-authored HTTP middleware) collides with `contributes.middleware` in the Plugin Contract Spec (Amendment 03 §6.5.3, plugin-contributed middleware — different mechanism, same word).
...
- **v0.2.1 — `plugins: [...]` field in `defineAihuConfig`.** The single most load-bearing addition: the config root admits a plugin array. `definePlugin({...})` factory ships behind `@aihu/plugin` package (Q8 collapse). v0.2 ships the **type contract and registration plumbing only** — the contract dispatches to a no-op compiler at first; v0.3+ wire the dispatcher to actual block parsers, macro lowerings, etc.
  - Packages: new `@aihu/plugin` (build/dev-time only; ~0 B runtime); `@aihu/server` config schema extension.
  - Runtime imports: `@aihu/plugin` runtime imports = none; build/dev-time only. `@aihu/server` config field accepts `Plugin[]`; runtime is unchanged.
  - Size-budget reservation: `@aihu/plugin` no runtime size row (build/dev only). `@aihu/server` no row (server-side, per v0.2.4).
  - Acceptance: `defineAihuConfig({ plugins: [forms({...})] })` parses; compiler accepts plugin registration without error; one example plugin (data — see v0.2.5) registers cleanly.
...
- **v0.2.2 — Parser stub for `@blockname { }` syntax (alongside existing `<script setup>` / `<template>` / `<style>` / `<agent>`).** Rust compiler gains a second `next_block()` discriminator — `@`-prefix vs `<`-tag. Both grammars accepted by the parser; lowering paths still go through the existing emit pipeline. **No deprecation yet** of `<script setup>` etc.
  - Packages: `@aihu/compiler` (Rust).
  - Runtime imports: none (compiler is build-time).
  - Acceptance: `tests/sfc_split.rs` adds dual-fixture coverage — one `.aihu` file with `<script setup>` + `<template>`, one with `@state {}` + `@template {}`. Both compile to the same emitted JS shape (function form when no agent/route blocks present). Compiler error message names which grammar was detected.

- **v0.2.3 — arbor 15 B regression cleanup.** Carry-over from R1 Scout Q1. Recover ≥ 15 B + ≥ 30 B safety margin via Compressor pass on `@aihu/arbor`. **Required before any v0.5 macro-element work** (which lowers to arbor primitives and would otherwise trip the size gate first).
  - Packages: `@aihu/arbor`.
  - Runtime imports: `@aihu/signals` (no change).
...
- **v0.2.6 — `@aihu/data` registered as canonical data plugin (no rename yet).** Install `@aihu/data` via `definePlugin({ namespace: 'data', contributes: {...} })` shim — registration only; dispatcher is no-op until v0.4 lowers `$resource`. Documents the data-as-plugin framing without churning the package name (Naming Scheme A rename to `@aihu-plugin/data` deferred to v1.0 cutover).
  - Packages: `@aihu/data` (adds plugin registration shim).
  - Runtime imports: `@aihu/signals`, `@aihu/context` (no change).
  - Acceptance: data plugin registers under `defineAihuConfig({ plugins: [data()] })` without error.
...
**Packages:** `@aihu/compiler` (Rust). **Runtime imports:** none (build-time).
...
- **v0.4.4 — Boolean-only attribute support (§1.4).** HTML `disabled`, aihu `$once`, `$raw`.
...
- **v0.4.6 — `@state` macros (Macro Vocabulary §2):** `$prop` (→ `defineProps<{...}>() + computed`), `$computed` (→ `computed()`), `$resource` (→ `data` plugin's `createResource`), `$effect` / `$effect.on` / `$watch` (→ `effect`), `$action` declaration form (→ `function name(...) { return batch(...) }`), `$lifecycle.{mount,dispose}` (→ `onMount` / `onCleanup` — **new exports from `@aihu/runtime`**).
...
- **v0.4.9 — Helper exports from `@aihu/runtime`:** `onMount`, `onCleanup`. ~30-50 B in runtime — runtime budget headroom is tight (currently +7 B at 1.14 kB / 1170 B). The C-6 typecheck pass deferred to v0.4 may push runtime past limit. **Pre-authorization (Polish Note 5):** the Architect's milestone plan **pre-authorizes a Compressor pass on `@aihu/runtime` during v0.4 macro-attribute lowering implementation, without re-surfacing to user**, as long as the pass stays within the Learning #42 split rationale (feature-bytes-vs-debt; recovered bytes are debt-side, new bytes for `onMount` / `onCleanup` are feature-side). The Compressor pass is a soft-gate that auto-resolves if recovery ≥ new feature cost. **Surface trigger fires only if recovery falls short of the new cost AND a runtime size-limit raise becomes necessary** (Learning #42 split formal raise) — see §"Risks and surface conditions" item 1. This pre-authorization narrows the surface scope to "raise needed" rather than "Compressor pass needed".
- **v0.4.10 — Conformance fixtures at `bench/compiler-conformance/template-attrs/` and `bench/compiler-conformance/macros/`.**

**Packages:** `@aihu/compiler`, `@aihu/runtime` (new lifecycle helper exports).
...
**Packages:** `@aihu/compiler` (lowering); `@aihu/runtime` (new exposed helpers `createShieldBoundary` etc., or kept as compiler-emitted SFC-internal — Architect-leaning: SFC-internal to avoid runtime size raise).
...
- **v0.6.1 — `@route` block parser + types.** New `BlockKind::Route` in `sfc.rs`; new `parser/route.rs` module parses TypeScript object literal body (`path`, `name?`, `middleware?`, `ssr?`, `layout?`). New `RouteBlock` struct, `route: Option<RouteBlock>` on `AihuSource`. Validation: `@route` outside `src/pages/` → compile error. `@layout 'admin'` shorthand recognized (defer drop to v1.1 per spec §12 Open Q).
...
- **v0.6.3 — Router consumes `@route` metadata.** `@aihu/router/src/vite-plugin.ts` `scanPages()` reads sibling `.route.json` files; `RouteDefinition` shape extended with `name?`, `middleware?`, `ssr?`, `layout?`. File-based scan stays as the fallback when no `@route` block present.
...
- **v0.6.5 — `build.target` field in `defineAihuConfig`.** New `build` object in `AihuConfig` (`packages/server/src/config.ts`); `target: BuildTarget`. Compiler reads via plugin context.
...
- **v0.6.7 — `$server` macro lowering.** Per Macro Vocabulary §2.12. Server file emits at `/server/_actions/{component-id}/{name}.ts`; client emits `createServerCall<Args, Return>('component-id/name')` stub. New `createServerCall` runtime helper in `@aihu/server` client subpath (or `@aihu/runtime` — Architect-leaning: `@aihu/server`'s client-safe subpath, since the wire format is server-defined).
- **v0.6.8 — File-based layouts (Q3:A).** `layouts/default.aihu`, `layouts/admin.aihu`. Compiler scans `src/layouts/`, emits `virtual:aihu-layouts`. Router resolves layout-then-page during match. `defineAihuConfig.layouts` map (default + per-route override). Per-page override via `@route { layout: 'admin' }`.
- **v0.6.9 — Conformance fixtures** at `bench/compiler-conformance/route/` and `bench/compiler-conformance/build-target/`.

**Packages:** `@aihu/compiler`, `@aihu/server` (config), `@aihu/router` (vite-plugin metadata read).
**Runtime imports:** `@aihu/server`'s new `createServerCall` is a small client-safe helper (~50-100 B); no new package deps. Router still imports `@aihu/server` (existing — `renderToString`).
**Size-budget reservation:** `@aihu/server` no row (server-side); `@aihu/router` stays at 1536 B (no new bytes — the `@route` consumption is build-time only). Layouts add no runtime bytes (compiler emits layout-wrapping code into page JS).
**Acceptance:** an `.aihu` page in `src/pages/admin/users.aihu` with `@route { name: 'admin-users', layout: 'admin', middleware: ['auth'] }` produces correctly-resolved router metadata; `--target client` build elides server artifacts with a warning; layouts render correctly nested.
...
- **v0.7.1 — `defineRouterMiddleware` + `composeRouterMiddleware` in `@aihu/router`.** Per Director Q6 Option 1. `RouteMatchContext { url, params, route, signal, env }`; `RouterResult { kind: 'continue' | 'redirect' | 'cancel', ... }`. ~150-220 B in `@aihu/router`. **Router size limit raise: +256 B (1536 → 1792 B).** Architect-leaning: pair with a Compressor pass (mirroring arbor's 176 B recovery) to net less than +256 B if possible; if Compressor recovers ≥ 100 B, the limit raise is +150-200 B instead.
...
- **v0.7.4 — Cross-package collision resolution (Naming Scheme A on Plugin Contract internals).**
  - `@aihu/server.createRouter` → renamed to `createRequestRouter` (or kept at `createRouter` under `@aihu/server` and the plugin-contributed `@aihu-plugin/router` namespace owns the file-route consumer; Architect-leaning: rename `@aihu/server.createRouter` to `createRequestRouter` for symmetry with `defineRequestHandler`).
  - `@aihu/server.defineMiddleware` → kept as the hand-authored HTTP middleware. Plugin-contributed middleware uses `contributes.middleware` (no naming clash because they're in different *kinds*: one is a function call, the other a config-object field). Naming Scheme A here is documentation: spec text says "plugin-contributed middleware vs hand-authored middleware".
  - `viteRouterPlugin` (`@aihu/router`) → renamed to `viteRouterIntegration` (disambiguates from "aihu plugin" — it IS a Vite plugin, NOT a `@aihu/plugin`).
  - `agentReadiness` (Vite plugin export from `@aihu/agent-readiness`) → renamed to `viteAgentReadinessIntegration`.
...
**Packages:** `@aihu/router` (+256 B raise), `@aihu/server` (rename `createRouter` → `createRequestRouter`), `@aihu/agent-readiness` (rename Vite plugin export).
**Runtime imports:** unchanged dep graph.
**Size-budget reservation:** `@aihu/router` raise to 1792 B. No other browser-package raises.
...
**Theme:** The framework is usable by a new consumer with `npx aihu app <name>`. Developer can ship a Hello World page (with `<$shield>`, `@route`, layout) end-to-end without reading the spec quartet.

**Items:**

- **v0.8.1 — `@aihu/cli` package.** Build-time Bun/Node CLI exposing `aihu app <name>`, `aihu page <route>`, `aihu component <name>`, `aihu plugin <name>`. Templated scaffolds for each.
- **v0.8.2 — Hello World template.** `npx aihu app my-app` produces:
  - `package.json` (deps: `@aihu/server`, `@aihu/router`, `@aihu/runtime`, `@aihu/arbor`, `@aihu/signals`, `@aihu/agent`; devDeps: `@aihu/cli`, `vite`)
  - `aihu.config.ts` (`defineAihuConfig({ build: { target: 'universal' }, plugins: [data(), agent()] })`)
  - `src/pages/index.aihu` (Hello World with `@template { Hello {{ name }} }`, `@state { $prop name: string = 'world' }`)
  - `src/layouts/default.aihu`
  - `vite.config.ts` (compiler + router + agent-readiness Vite integrations)
- **v0.8.3 — First-run UX.** `bun run dev` starts the Vite dev server with HMR; navigating to `localhost:5173` renders Hello World; making an edit to `index.aihu` HMR-updates without page reload.
- **v0.8.4 — Light-off procedure docs.** Documentation page describing the first-run experience and the dev → build → preview cycle.
- **v0.8.5 — Plugin scaffold template.** `npx aihu plugin my-plugin` produces a skeleton npm package with `definePlugin({ name, version, namespace, contributes: {} })`, an example macro, an example block, and a publishable `package.json`.

**Packages:** new `@aihu/cli` (build/dev-time only; ~0 B runtime impact).
**Runtime imports:** none.
**Size-budget reservation:** none new (CLI is build-time).
**Acceptance:** `npx aihu app demo && cd demo && bun install && bun run dev` produces a running Hello World; an end-to-end smoke test in CI runs through this on every PR.
...
- **v0.9.1 — `docs/site/` Markdown build pipeline.** Static site generator (handrolled or via the v0.6 SSG path; Architect-leaning: handrolled for v0.9, since SSG is itself part of the v1.0 surface and must not depend on under-test code). Pages:
  - **Introduction** — what aihu is, why it exists, the v3 dep-free thesis.
  - **Installation** — `npx aihu app`, prerequisites, package versions.
...
- **v0.9.4 — HMR aihu-native confirmation.** Re-run grep gate: `grep -r '@vitejs/client' packages/*/src/` zero hits. (Already PASS per R2 audit.)
- **v0.9.5 — Cross-runtime adapter completeness tests.** Confirm `@aihu/server` runs across Bun, Deno, Node, Cloudflare Workers, Vercel Edge with the existing 3-state loader. Parity matrix in CI.
- **v0.9.6 — Build-tool independence smoke test.** Verify the compiler emits code that runs WITHOUT Vite (consumed via raw `tsc` + a static file server). Acceptance gate for the abstract `@aihu/plugin` (Q8 collapse).
...
- **v1.0.7 — Dual-grammar deprecation removal.** Compiler removes the `<script setup>` / `<template>` / `<style>` / `<agent>` HTML-tag parser path. Only `@blockname { }` grammar accepted from v1.0 onward. Migration tool (`npx aihu migrate`) ships in `@aihu/cli` to auto-convert v0.1.x SFCs.
...
- **v1.0.9 — Naming Scheme A rename publication.** `@aihu/data` → `@aihu-plugin/data`; `@aihu/agent-service` → `@aihu-plugin/agent-service` (or kept core; decided in §"Naming Scheme A application"). `@aihu/agent-readiness` → `@aihu-plugin/agent-readiness`. Old package names publish a single `1.0.0` "moved" stub that re-exports from the new home.
...
**Acceptance:** `git tag v1.0.0` published; consumer can `npx aihu app demo` against published packages; the framework is what the spec quartet says it is.
...
|---|---|---|---|---|---|---|
| `@aihu/signals` | zero | zero | zero | zero | zero | clean throughout |
| `@aihu/arbor` | `@aihu/signals` | unchanged | unchanged | unchanged | unchanged | clean |
| `@aihu/runtime` | peer: `@aihu/arbor`, `@aihu/signals` | unchanged | unchanged | unchanged | unchanged | clean |
| `@aihu/context` | zero | unchanged | unchanged | unchanged | unchanged | clean |
| `@aihu/agent` | zero | unchanged | unchanged | unchanged | unchanged | clean |
| `@aihu/data` | `@aihu/signals`, `@aihu/context` | + plugin shim | unchanged | unchanged | rename → `@aihu-plugin/data` (v1.0.9) | clean |
| `@aihu/router` | `@aihu/server` | unchanged | + size raise +256 B (v0.7.1) | unchanged | unchanged | hand-rolled matcher; isomorphic middleware in router scope only |
| `@aihu/server` | `@aihu/agent`, optional native addons | + `plugins` config field (v0.2.1), + `build` config (v0.6.5) | + Amendment 03 §6.5 wiring (v0.7.3), + `createServerCall` (v0.6.7) | unchanged | unchanged | clean (native addons stay under `@aihu/*`) |
| `@aihu/agent-service` | `@aihu/agent` | unchanged | unchanged | unchanged | unchanged | clean |
| `@aihu/agent-readiness` | `@aihu/server`, `@aihu/agent` | unchanged | + Vite-plugin rename (v0.7.4) | unchanged | rename → `@aihu-plugin/agent-readiness` (v1.0.9) | clean |
| `@aihu/compiler` | peer: `vite ≥5` (optional) | + `@`-block parser (v0.2.2) | + `@route` (v0.6.1), + build target (v0.6.4), + macro lowerings (v0.4) | unchanged | + dual-grammar removal (v1.0.7) | peer becomes optional-only by v1.0 |
| `@aihu/plugin` (NEW v0.2.1) | n/a | runtime: none | unchanged | unchanged | unchanged | build/dev-time only |
| `@aihu/cli` (NEW v0.8.1) | n/a | n/a | n/a | runtime: none | unchanged | build/dev-time only |
| `@aihu/image` (DEFERRED — see assets follow-up) | n/a | n/a | n/a | n/a | possibly v1.0 if assets in baseline; see follow-up | Sharp build-only |
| `@aihu/fonts` (DEFERRED) | n/a | n/a | n/a | n/a | possibly v1.0 | fontkit build-only |
| `@aihu/css-pipeline` (DEFERRED) | n/a | n/a | n/a | n/a | possibly v1.0 if assets in baseline | postcss build-only or compiler-internal |

**No package carries any non-`@aihu/*` runtime dep at any version.** All non-`@aihu/*` deps remain build-time / dev-time / peer-optional. Native addons (`@aihu/server-{platform}`) stay under the `@aihu/*` namespace.
...
| `@aihu/agent-service` Plan 5.3 wiring (`handleToolCall()`) | R1 Scout Q15 | **v0.4.8** (`@agent` macros) — `handleToolCall` dispatches to spec-mandated agent metadata |
...
- **`@aihu/data` → `@aihu-plugin/data`** — data is the canonical data plugin per Plugin Contract Spec §0 / §9.3. Keep public APIs (`createResource`, `createResourceStore`, etc.) unchanged; the namespace move is the rename.
- **`@aihu/agent-readiness` → `@aihu-plugin/agent-readiness`** — already a Vite plugin + content-negotiation handler; fits plugin-side cleanly.
- **`@aihu/agent-service`** — Architect-leaning: keep core (it's the MCP wire transport, not a plugin contribution). Final call: ratify at v1.0.9 cutover; if user prefers full Scheme A coverage, rename to `@aihu-plugin/agent-service`.
...
- `definePlugin`, `contributes`, `lowering`, `BuildContext`, `SfcContext`, `BlockContext`, `MacroContext`, `LoweringResult`, `serverRuntime`, `serverOnly`, `target` (in LoweringResult), `parsers`, `transforms`, `hooks`, `dependencies` (plugin field), `aihuVersion`, `componentAliases`, `validIn` — **all kept as-is**. The `@aihu-plugin/*` scope itself signals "plugin world"; per-type prefixing would be redundant.

### Cross-package collision resolution

- **`createRouter` collision** — rename **`@aihu/server.createRouter` to `createRequestRouter`**. Rationale: `@aihu/server`'s router is the request-router for the HTTP envelope; `@aihu/router`'s `createRouter` is the file-based route table consumer. The `Request` discriminator is the natural disambiguation. Spec ratification (Q8 collapse) absorbs this rename. Lands at **v0.7.4** (alongside the router middleware work, which is the natural moment).
- **`defineMiddleware` overload** — keep `@aihu/server.defineMiddleware` (hand-authored HTTP middleware) and `contributes.middleware` (plugin contract field). They are different *kinds* of thing (function call vs config-object field), so no rename is forced. Document the distinction in the Plugin Contract Spec at v1.0.6 migration.
- **`viteRouterPlugin` / `agentReadiness`** — rename to **`viteRouterIntegration`** / **`viteAgentReadinessIntegration`**. Disambiguates from "aihu plugin" (these are Vite plugins, not aihu plugins). Lands at **v0.7.4**.

### What Scheme A does NOT touch

- `@aihu/signals`, `@aihu/arbor`, `@aihu/runtime`, `@aihu/context`, `@aihu/server`, `@aihu/router`, `@aihu/agent`, `@aihu/compiler`, `@aihu/cli` — all stay core (not plugin-contributed). User-facing surface (`signal`, `computed`, `effect`, `branch`, `leaf`, `mount`, `defineComponent`, `defineElement`, `defineRoute`, `defineApiRoute`, `defineLoader`, `defineAihuConfig`, `getAgentMetadata`, `registerAgentMetadata`) — all kept unchanged.

---

## Open assets-package-design follow-up

The assets-package-design-stub at `.team/v1-reconciliation/assets-package-design-stub.md` (Architect R2 companion) describes `@aihu/image` / `@aihu/fonts` / `@aihu/css-pipeline`. Under the original v1/v1.5/v2/v3 milestone shape, these targeted v1.5. **Under the locked 0.2 → 0.9 → 1.0 shape, the question is whether assets are in the v1.0 baseline or deferred.**
...
1. **Parser dual-grammar period (v0.2 → v1.0).** Carrying both `@blockname { }` and `<script setup>` / `<template>` / `<style>` / `<agent>` parsers from v0.2 to v1.0 is ~6 months of compiler complexity. Mitigations: clear deprecation warnings from v0.3 onward; auto-conversion tool in `@aihu/cli` (v0.8); v1.0 hard removal lands on a clean cut.
...
4. **Plugin Contract surface is entirely new at v0.2.** First plugin (`@aihu/data` registration shim) is a smoke test for the contract. If the contract shape is wrong, v0.2 detects it; v0.3+ fixes are still cheap. By v0.7 (server-side contributions wiring), the surface should be stable.
5. **`@aihu/runtime` size headroom (7 B) tight for v0.4 lifecycle helpers.** `onMount`/`onCleanup` exports may push runtime past 1170 B. Mitigation: Compressor pass on runtime; if recovery falls short, Learning #42 split (feature-bytes-vs-debt) per `.size-limit.json` row comment. **Surface trigger if Compressor recovers < 30 B.**
6. **Naming Scheme A rename publication at v1.0.9.** Renaming `@aihu/data` to `@aihu-plugin/data` is a breaking change. Mitigation: stub re-export package at old name; `npx aihu migrate` updates user `package.json`.

### Surface conditions

The Architect surfaces back to the Director (and from Director to user) on:

1. **`@aihu/runtime` Compressor recovery falls short of `onMount`/`onCleanup` cost (v0.4.9).** Decision needed: raise runtime limit (Learning #42 split) or relocate helpers (e.g., to `@aihu/arbor` if it has headroom post-v0.2.3). Surface includes a recommendation.
2. **`@aihu/router` Compressor recovery falls short of router middleware cost (v0.7.1).** Currently +256 B reserved; if Compressor recovers < 100 B, raise stays at +256 B; if < 50 B, surface to consider scope reduction (e.g., drop file-convention auto-wire to v1.x).
...
| **Q3:A** | File-based layouts (`layouts/default.aihu`, Nuxt-style) | User (this session) |
...
| **Q8 collapse** | Plugin Contract Spec ratified; `@aihu/plugin` IS the abstract surface | User (this session); migrates to `docs/superpowers/specs/` at v1.0.6 |
...
| **Q6 router middleware: Option 1** | Router-level isomorphic `defineRouterMiddleware`; +256 B `@aihu/router` raise | Director Q6 research (recommendation) → user single-click ratification (this session) |
...
| **Naming Scheme A** | `@aihu-plugin/*` namespace for plugin-contributed packages; core APIs unchanged | Scout R2.5 (recommendation) → user (this session) narrowed scope |
...
2. **Full `@aihu/assets` design.** Companion stub stays; full design is a post-v1.0 design session (per §"Open assets-package-design follow-up").
3. **`@aihu/magna` design.** Out of scope for v1.0. Magna integration is a post-v1.0 milestone — `@aihu/data`'s `Resource<T>` shape is **frozen for v1.0 wire compatibility** so magna can layer on top in v1.x without re-opening v1.0 surface.
