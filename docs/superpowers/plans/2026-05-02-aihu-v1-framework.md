...
- `definePlugin`, `contributes`, `lowering`, `BuildContext`, `SfcContext`, `BlockContext`, `MacroContext`, `LoweringResult`, `serverRuntime`, `serverOnly`, `target` (in LoweringResult), `parsers`, `transforms`, `hooks`, `dependencies` (plugin field), `scribeVersion`, `componentAliases`, `validIn` — **all kept as-is**. The `@aihu-plugin/*` scope itself signals "plugin world"; per-type prefixing would be redundant.
