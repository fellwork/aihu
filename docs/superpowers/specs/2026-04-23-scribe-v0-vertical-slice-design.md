...
**This spec is the vertical slice** — the smallest end-to-end pipeline that proves the core thesis holds: a hand-authored `.scribe` SFC compiling through our own Rust compiler, loaded by Vite, and running in a real browser as a reactive custom element.
...
1. Author a `.scribe` single-file component with four sections (`<template>`, `<script>`, `<style>`, `<agent>`) and render it reactively in Chromium via the Vite dev server.
...
fellwork/scribe/
├── crates/
│   ├── scribe-parser/         # SFC parsing, OXC-powered
│   ├── scribe-compiler/       # AST→AST emitter producing WC class JS
│   └── scribe-vite-plugin/    # napi-rs bridge + Rolldown-native hooks
├── packages/
│   ├── @scribe/signals/       # pure signal primitives (~1 KB gz)
│   ├── @scribe/arbor/         # reactive tree: branch/leaf/mount (~2 KB gz)
│   ├── @scribe/runtime/       # WC helpers, defineElement (~1 KB gz)
│   ├── @scribe/agent/         # getAgentMetadata accessor
│   └── @scribe/dev/           # Vite preset wiring it all together
├── examples/
│   └── hello-scribe/          # v0 hello-world, primary integration test
├── docs/
...
### 6.3 Primitives (in `@scribe/arbor`)
...
| Package | Target (gz) |
|---|---|
| `@scribe/signals` | ~1.0 KB |
| `@scribe/arbor` | ~2.0 KB |
| `@scribe/runtime` | ~1.0 KB |
| `@scribe/agent` | negligible |
| **Total** | **≤ 4.0 KB** |

For comparison: minimal React hello-world ~44 KB, Vue 3 ~40 KB, Preact ~4.5 KB, Solid ~7 KB, Svelte ~2 KB, Qwik ~1 KB initial.

**Gate condition:** CI fails if the combined minified+gzipped size of `@scribe/signals` + `@scribe/arbor` + `@scribe/runtime` + `@scribe/agent` exceeds **4096 bytes**. Budget changes require an explicit PR against `.size-limit.config`.

## 7. Components of the v0 slice

### 7.1 `scribe-parser` (Rust crate)

Consumes `.scribe` bytes. Returns:
...
### 7.2 `scribe-compiler` (Rust crate)
...
Knows nothing about Vite. Knows nothing about the runtime's internals beyond the public API of `@scribe/arbor` + `@scribe/signals` + `@scribe/runtime`.

### 7.3 `scribe-vite-plugin` (Rust + napi-rs)

Transforms `.scribe` imports. Hooks Rolldown-native where available; falls back to JS-shaped hooks for the Vite dev-server middleware path. Owns HMR for `.scribe` files — component replacement, best-effort state preservation. Reports structured diagnostics to the Vite overlay.
...
import { scribe } from '@scribe/dev'
export default defineConfig({ plugins: [scribe()] })
```

### 7.9 `examples/hello-scribe`

Single `.scribe` SFC with all four sections. Counter state, button, interpolation. Doubles as the primary Playwright integration test.

## 8. Data flow

Life of a `.scribe` file:

```
.scribe file bytes
      ▼
scribe-parser          → SfcDescriptor { template, script, style, agent }
      ▼
scribe-compiler        → OxcProgram (JS module AST)
      ▼
OXC codegen            → JS source string + sourcemap
      ▼
Vite / Rolldown bundle (with @scribe/runtime family imports resolved)
      ▼
Browser loads module   → customElements.define registers tag
      ▼
Page contains <hello-scribe>
      ▼
...
- Sourcemaps land in the original `.scribe` file, not generated JS.
...
Codes are stable, documented, link-resolvable. Rendered in the Vite dev overlay with the original `.scribe` source.
...
- Build errors → Vite overlay, sourcemapped into `.scribe`.
...
- **Integration:** Vite plugin harness (real plugin, ephemeral Vite config, `.scribe` fixtures); cross-package mount tests in JSDOM.
- **Browser (Playwright + headless Chromium):** `examples/hello-scribe` is the primary e2e — click increments, only the text node mutates, unmount cleans up.
...
- **Bundle size gate**: fails if the combined minified+gzipped size of `@scribe/signals` + `@scribe/arbor` + `@scribe/runtime` + `@scribe/agent` exceeds 4096 bytes. Budget changes require an explicit PR against `.size-limit.config`.
...
1. `examples/hello-scribe` runs in Chromium via Playwright; click increments counter; only the text node mutates.
2. All CI gates pass: cargo test, clippy, vitest, Playwright, bundle-size ≤ 4096 bytes gz, insta snapshots clean.
3. The `<agent>` block round-trips through the static export and is retrievable via `getAgentMetadata('hello-scribe')`.
4. A deliberately-malformed `.scribe` file produces a structured diagnostic rendered in the Vite overlay.
...
- "Make `@scribe/signals` a generic Observable library" → no, keep it scribe-focused.
...
1. Signal library foundation — subsumed into this spec as `@scribe/signals`.
2. SFC parser — subsumed as `scribe-parser`.
3. arbor runtime + lifecycle — subsumed as `@scribe/arbor`.
