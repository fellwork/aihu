...
> **Tag naming.** Component tags must normalize to a hyphenated custom-element name: multi-word PascalCase kebab-cases automatically (`<UserCard>` → `user-card`), hyphenated tags pass through lowercased, and **single-word component names are a hard compile error (C450)** — use a hyphenated tag (e.g. `<x-comment>`) or an explicit hyphenated `@meta name`.
