...
**Author:** Architect
**Status:** APPLIED 2026-05-02 (v1 reconciliation session; Builder R4 migration) — **Option A (provisional middleware §6.5.3) locked** per user adjudication
**Applied to:** `docs/superpowers/specs/2026-05-02-spec-plugin-contract.md` §6.5 (new section between §6 and §7)
**Depends on:** Amendment 02 (Block Structure §11.5) — applied first as required
...
Add a note that middleware design may evolve in the v1.x series. Plugin authors using middleware should pin to minor aihu versions (`^1.x.0`) for stability.
...
4. **Build target propagation** — the build target flows from `aihu.config.ts` through every plugin lowering call
