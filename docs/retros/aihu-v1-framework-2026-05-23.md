...
| **CI fix — Plan 2/Track B binary-build gate** | r11 | Added to the `plan-a.yml` `check` job: workspace-root `cargo build --release` (emits BOTH `aihu-compile` + `aihu-css-compile`), staged the fresh `aihu-compile` into `packages/compiler/bin/`, `AIHU_SKIP_POSTINSTALL=1` so postinstall does not clobber it, and an `actions/cache@v4` cargo cache. plan-a.yml only, 31 insertions, purely additive. Both e2e suites pass 8/8 (Track B's 3 + Plan 2's 5). | `feat/css-2-ast-scanner`, commit `9bbdd05` | `b92cb214-6cb9-497a-b8c2-8c141a202306` | (closes the r10 NEEDS_FIX) |
