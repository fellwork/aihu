...
- `packages/css-engine/tests/sfc-e2e.test.ts` (Plan 2) calls `compileSfc` → `compileToAst` (`packages/compiler/js/index.ts`), which spawns **`aihu-compile --ast-json`**, resolved from **`packages/compiler/bin/`** (the postinstall location, `AIHU_COMPILE_BIN ?? ../bin/aihu-compile{ext}`) — NOT `target/release`.
...
3. Set **`AIHU_SKIP_POSTINSTALL: "1"`** on the `bun install` step (confirmed env var at `packages/compiler/js/postinstall.ts:224`) so postinstall does NOT clobber the freshly staged binary with the stale download. Order it so the install step does not overwrite the staged binary.
...
Future css-engine packages (Plans 3-6) that add e2e suites spawning these binaries inherit the same requirement — the `check` job already builds both, so no change is needed as long as the cargo-build + stage + `AIHU_SKIP_POSTINSTALL` steps stay in place.
