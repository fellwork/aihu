...
| lesson | one line |
|---|---|
| [An absent value rendered as real, failing quietly](absent-value-rendered-as-real.md) | Something that does not exist gets formatted as a plausible, often flattering value. **39 instances.** An absence must be loud — and never *assert* that an empty result is trustworthy unless you can prove it. |
