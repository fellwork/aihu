...
| lesson | one line |
|---|---|
| [An absent value rendered as real, failing quietly](absent-value-rendered-as-real.md) | Something that does not exist gets formatted as a plausible, often flattering value. **51 instances** — plus **the mirror**, a *present* value rendered as absent, which is worse because a false absence makes you act immediately on the wrong target. An absence must be loud; and before believing an instrument that reports nothing, prove it can see something. |
| [The thing being checked is not the thing that changed](checked-thing-is-not-the-changed-thing.md) | An honest measurement of the wrong artifact — a published copy, `src` instead of `dist`, another branch, another machine, or a config comment instead of the config's behaviour. **29 instances**, incl. one that took three instruments to settle and one where I verified a fix against my worktree instead of the commit. Red and green are equally uninformative. |
...
| [A green `ci-ok` can certify a build that never ran](ci-ok-green-can-certify-a-build-that-never-ran.md) | The required context is an aggregate, and one bit cannot say *which run produced it*. **Four faces in one session** — stale-green, green-beside-in_progress, red-because-cancelled, draft-gated — all the same shape: **"did not fail" read as "passed."** `gh pr checks` collapses the runs and structurally cannot answer; `bun scripts/ci-receipt.ts <pr\|sha>` can, and refuses rather than passing vacuously. |
