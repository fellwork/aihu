...
- `.team/v1-reconciliation/assets-package-design-stub.md` (Round 2 — `@aihu/image` / `@aihu/fonts` / `@aihu/css-pipeline`)

**Director adjudications already in:**
- Spec quartet ratification = v1.0-gating
- Plugin Contract `contributes` / `lowering` / `BuildContext` IS the abstract `@aihu/plugin` (Q8 collapse)
- Amendment 02 path convention: **Option A (`_aihu-server/` prefix)**
...
**Defines:** the file format for `.aihu` SFCs at the structural level — which blocks exist, how they delimit, parse, and compose. Authoritative contract between SFC source and the compiler's top-level parser.
...
**File-system conventions (§7):** `src/pages/` -> page component (auto-routed); `src/components/`, `src/layouts/`, `src/composables/` per role. Path-based namespace prevents component name collisions (§7.4). Layouts attached via `aihu.config.ts` `layouts` map or per-page override `@route { layout: 'admin' }` / shorthand `@layout 'admin'`.
...
**§11.5 (Amendment 02, Option A applied):** SPLIT-BUNDLE COMPILATION — `$server`, `<form $action>`, and `@agent` block emit two artifacts each (server file under `_aihu-server/{actions,form-actions,mcp}/{component-id}/...` + client RPC stub or no-client). Build target field `client | server | universal` introduced; default `universal` for pages; `client` mode warns-and-elides server-only macros.
...
**Identifier resolution (§2):** lookup order — local `@state` decls -> slot-exposed context (`shield.error`, `guard.user`, etc.) -> plugin-contributed values -> `aihu.config.ts` imports. Computed/bracket/call access requires curly form.
...
**Migration tables (§10):** Vue/React/Svelte -> aihu one-to-one mapping. Vue's `:class`, `@click` -> aihu's `class={...}`, `$on:click="..."`.
...
**Cross-references:** quartet member; consumes Macro Vocabulary + Plugin Contract; depends on `@aihu/{signals,arbor,runtime}` stable.
...
- `$resource` -> `createResource(keyFn, fetcher)` (consumes `@aihu/data`)
- `$effect` / `$effect.on` / `$watch` -> `effect()` (consumes `@aihu/signals`)
- `$action` -> `function name(...) { return batch(() => {...}) }` (consumes `@aihu/signals.batch`)
- `$lifecycle.{mount,dispose}` -> `onMount` / `onCleanup` (consumes `@aihu/runtime`)
- `$shared` -> `useSharedState(key, default)` (NEW HELPER — not in current runtime)
- `$cookie` -> `useCookie(name, opts)` (NEW HELPER)
- `$server` -> split-bundle artifact pair: server file at `_aihu-server/actions/{component-id}/{name}.ts` + client `createServerCall<Args, Return>('component-id/name')` (per Amendment 02 §11.5)
- `$meta` -> `useHead({...})` (NEW HELPER)
- `$if` -> `createIfBoundary(...)` from `@aihu/arbor` (current arbor exports `when` not `createIfBoundary`)
...
- **Components:** plugin ships `.aihu` files in its `components/` dir; auto-registered project-wide. Project-local `src/components/Input.aihu` shadows plugin.
- **Transforms:** `{ stage: 'after-parse' | 'before-lower' | 'after-lower', fn }` — AST/output mutation.

**Configuration (§3):** `aihu.config.ts` -> `plugins: [forms({...}), data({...})]`. `configSchema` declares accepted shape; compiler validates at startup. Hooks/parsers receive resolved config via context.
...
|---|---|---|---|
| §1.1 | `.aihu` extension recognized | SHIPPED | `packages/compiler/src/parser/sfc.rs` parses `.aihu` SFCs; `bin/aihu-compile` accepts `.aihu` |
...
| §7.1 | `src/pages/` -> page; `src/components/` -> component; `src/layouts/`; `src/composables/` | PARTIAL | `@aihu/router` (`packages/router/src/vite-plugin.ts`) scans `src/pages/`, emits `virtual:aihu-routes`. Roles `components`/`layouts`/`composables` not enforced by compiler |
| §7.2 | Routing inference from path: `index.aihu`, `[id].aihu`, `[...catchAll].aihu` | SHIPPED | `packages/router/src/router.ts:matchRoute` handles static / param / catchall priority |
| §7.3 | `@route { path, name, middleware, ssr, layout }` block override | **GAP** | Compiler has no `@route` block. Router (`@aihu/router`) is file-based scan only — no SFC-level route override consumed |
| §7.5 | Layouts via `aihu.config.ts` `layouts: { default, routes }` | **GAP** | `defineAihuConfig` (`packages/server/src/config.ts:23-27`) has no `layouts` field. No layout slot in router |
...
| **§11.5 (AMD-02)** | Split-bundle for `$server` / `<form $action>` / `@agent` -> server artifact + client stub | **GAP** | Compiler emits a single `EmitResult { js, manifest_json }` (`packages/compiler/src/codegen/emit.rs:4-8`); manifest_json is the agent manifest, NOT a `_aihu-server/mcp/{component-id}.ts` server artifact. No `$server` macro support. No `<form $action>` lowering |
| **§11.5** | Build target: `client` / `server` / `universal` | **GAP** | No `build.target` field anywhere; `defineAihuConfig` lacks it; no compiler flag honoring it. Confirmed by grep — zero hits across packages |
| §11.5 | Server output path: `_aihu-server/{actions,form-actions,mcp}/{component-id}/{name}.ts` (Option A) | **GAP** | No code path emits to this prefix |
...
| §1.4 | Boolean-only attrs (HTML `disabled`, aihu `$once`/`$raw`) | PARTIAL | Static attrs supported (`directives.rs:42-46`); aihu-specific boolean macros `$once`/`$raw` don't exist |
...
| 2.2 | `$computed` | **GAP** | No `$computed` keyword. `@aihu/signals.computed` exists at runtime (`packages/signals/src/computed.ts`) but no compiler-level macro |
| 2.3 | `$resource` | **GAP** | `@aihu/data.createResource` shipped (`packages/data/src/resource.ts`); no `$resource` macro lowering |
| 2.4 | `$effect` | **GAP** | `@aihu/signals.effect` shipped; no `$effect` macro |
...
| 2.8 | `$lifecycle.{mount,dispose}` | **GAP** | `@aihu/runtime` has lifecycle implicit in `connectedCallback`/`disconnectedCallback` but no `onMount`/`onCleanup` user-facing helpers exposed (search: `grep onMount\|onCleanup packages/runtime/src` — zero hits) |
| 2.9 | `$expose` (`@state`) | **GAP** | No `defineExpose` mechanism in `@aihu/runtime` (`packages/runtime/src/index.ts` exports only `defineComponent`/`defineElement`/internals) |
| 2.10 | `$shared` | **GAP** | No `useSharedState` helper anywhere |
| 2.11 | `$cookie` | **GAP** | No `useCookie` helper |
| **2.12** | **`$server`** | **GAP** | No `$server` macro; no split-bundle artifact emission; no `createServerCall` runtime helper. `@aihu/server` has `defineApiRoute`/`defineLoader` (`packages/server/src/{api,data}.ts`) but those are server-side only — not RPC stub generation per §11.5 |
...
| 4.2 | `$tokens` | **GAP** | No design-token pipeline; `aihu.config.ts` has no `style.tokens` field (`packages/server/src/config.ts`) |
...
|---|---|---|---|
| 1.1 | `definePlugin({...})` factory exported from `@aihu/plugin` | **GAP** | No `@aihu/plugin` package exists. `ls packages/` confirms 11 packages, none named `plugin` |
...
| 3 | `aihu.config.ts` `plugins: [...]` array | **GAP** | `defineAihuConfig` (`packages/server/src/config.ts:23-27`) `AihuConfig` interface has only `server`, `agent`, `routes` — NO `plugins` field |
...
| **6.5.3** | `middleware: [{ name, stage, handler }]` PROVISIONAL | **GAP** | `@aihu/server.defineMiddleware` exists (`packages/server/src/middleware.ts:3-15`) for hand-authored middleware, NOT plugin-contributed — different mechanism |
...
| `middleware` (in `contributes`) | NO | **AMBIGUOUS** | Collides with `defineMiddleware` in `@aihu/server` — different concept, same name. Alt: `pluginMiddleware` or `middlewareContributions` |
...
Walked the public exports of all 11 packages on `7fa0957` plus the spec-implied `@aihu/plugin`.

| API | Package | Plugin-infra connection? | Naming reveals? | Suggested rename / namespace |
|---|---|---|---|---|
| `defineComponent` | `@aihu/runtime` | NO (user-facing) | clear (matches Vue) | Keep |
| `defineElement` | `@aihu/runtime` | NO (compiler-emitted) | partial — looks like a user API but is internal-ish (Learning #12 split) | Keep, document better |
| `defineMiddleware` | `@aihu/server` | NO (user-defines hand-authored MW) | **AMBIGUOUS** — collides with Plugin Contract `contributes.middleware` (Amendment 03) | Disambiguate — `defineServerMiddleware` for hand-authored vs `pluginMiddleware` for plugin contribution |
| `composeMiddleware` | `@aihu/server` | NO | clear | Keep |
| `defineRoute` | `@aihu/server` | NO | partial — collides with `@aihu/router.createRouter`'s `RouteDefinition` | Disambiguate — `defineServerRoute` / `defineRouteHandler` |
| `createRouter` | `@aihu/server` AND `@aihu/router` | NO | **NAME COLLISION** — same identifier, two packages | Disambiguate — server's is request-router, router's is file-based-route-table |
| `defineApiRoute` | `@aihu/server` | NO | clear | Keep |
| `defineLoader` | `@aihu/server` | NO | partial — could be plugin-contributed in v2; doesn't reveal | Keep, or `defineRouteLoader` |
| `defineAihuConfig` | `@aihu/server` | YES (config root for plugins per spec) | **AMBIGUOUS** — name says "aihu config" but doesn't reveal it's the plugin-registration surface | Keep, but ratification must add `plugins: [...]` field |
| `viteRouterPlugin` | `@aihu/router` | partial — IS a Vite plugin, NOT a `@aihu/plugin` | **AMBIGUOUS** — "plugin" overloaded | Disambiguate — `viteRouterIntegration` or note "this is a Vite plugin, not a aihu plugin" |
| `agentReadiness` (Vite plugin) | `@aihu/agent-readiness` | partial — same overload | **AMBIGUOUS** | Same — `viteAgentReadinessPlugin` or `agentReadinessVite` |
| `createResource` / `createResourceStore` / `ResourceStoreToken` / `createResourceSerializer` | `@aihu/data` | YES per spec (data is plugin-contributed) but exported as core | **AMBIGUOUS** | If data IS the canonical data plugin per Plugin Contract Spec §9.3, these should live behind `@aihu-plugin/data` not `@aihu/data` |
| `createAgentService` | `@aihu/agent-service` | NO (MCP wire) | clear | Keep |
| `getAgentMetadata` / `registerAgentMetadata` | `@aihu/agent` | NO (registry) | clear | Keep |
| `generateLlmsTxt` / `generateLlmsFullTxt` / `generateMcpServerCard` / `generateRobotsTxt` / `createContentNegotiationHandler` / `createAgentReadinessRoutes` | `@aihu/agent-readiness` | NO | clear | Keep |
| `signal` / `computed` / `effect` / `batch` / `untrack` / `$state` | `@aihu/signals` | NO (core primitive) | clear | Keep |
| `branch` / `leaf` / `mount` / `hydrate` / `slot` / `when` / `each` | `@aihu/arbor` | NO (core primitive) | clear | Keep |
| `definePlugin` (spec-implied) | `@aihu/plugin` (GAP) | YES | clear | When package lands |

**Findings count: 9** ambiguous or colliding APIs across packages. The two biggest naming clashes:
1. `createRouter` exists in both `@aihu/server` and `@aihu/router` — same name, different intent.
...
#### Scheme A — "Plugin contributions namespace under `@aihu-plugin/*`; core APIs unchanged"

**Premise:** The user-facing surface (`defineComponent`, `defineRoute`, `signal`, `branch`, etc.) is fine and shipped. The naming ambiguity is *strictly* in the plugin contract internals + the data/agent/forms-as-plugin reframing. Move every plugin-contributed package under the `@aihu-plugin/*` scope per Plugin Contract Spec §3 examples (`@aihu-plugin/forms`, `@aihu-plugin/data`, `@aihu-plugin/agent`).

**Rename pass:**
- `@aihu/data` -> `@aihu-plugin/data` (when ratified as the canonical data plugin)
- `@aihu/agent-service` -> `@aihu-plugin/agent` (or keep agent core; agent-readiness goes plugin-side)
- `@aihu/agent-readiness` -> `@aihu-plugin/agent-readiness` (it's already a Vite plugin + content-negotiation handler — fits plugin-side)
- Plugin Contract internals stay un-renamed (`contributes`, `lowering`, `BuildContext`) — the `@aihu-plugin/*` scope itself signals "this is the plugin world."

**Tradeoff:** Forces a v1-cutover npm rename for `@aihu/data`, `@aihu/agent-service`, `@aihu/agent-readiness`. Big breaking change. Aligns the codebase with the Plugin Contract Spec's framing 1:1.
...
3. Disambiguates the `defineMiddleware` collision: hand-authored MW stays in `@aihu/server`; plugin-contributed MW lives in `@aihu-plugin/*` and uses `contributes.middleware` (no naming clash because they're in different scopes).
4. The user's critique resolves: a reader sees `import x from '@aihu-plugin/forms'` and immediately knows the API surface is plugin infrastructure.
...
7. **CompileError rendering format vs spec error template (Template Attribute Syntax §7).** Spec specifies caret-pointing source-line context. Current `CompileError` carries `{ message, line, col, code }` but rendering format is unknown without seeing `bin/aihu-compile`. Investigator: read `packages/compiler/bin/` to confirm rendering matches or diverges.

8. **`@aihu/data` future scope: stay core, or move to `@aihu-plugin/data` per Scheme A?** Plugin Contract Spec §9.3 frames data as a plugin example. Current `@aihu/data` ships as core (Round 1 Scout §1.2). Decision needed before §3d Scheme A can be ratified.
...
Section 3 (naming audit):
  ...
  - Coherence pass recommendation: Scheme A — "Plugin contributions namespace under @aihu-plugin/*; core APIs unchanged"
