...
- **Sizes:** `bun run size` → **7 of 8 packages within budget; `@scribe/arbor` is 15 B OVER (regression).** Full table below.
- **Packages on main:** 11 (`agent`, `agent-readiness`, `agent-service`, `arbor`, `compiler`, `context`, `data`, `router`, `runtime`, `server`, `signals`).
- **`.size-limit.json` rows:** 8 (no entry for `@scribe/server` or `@scribe/agent-readiness`).
- **`@scribe/server` ships a Rust SSR addon** (`packages/server/src-native/`, napi-rs cdylib) plus a 3-state loader (`packages/server/src/loader.ts`); `renderToString` is exported from `loader.ts`, `renderToStream` from `ssr.ts`.

### 1.2 Per-package surface

| Package | Size | Limit | Headroom | Status | Public API (value exports) |
|---|---|---|---|---|---|
| `@scribe/context` | 249 B | 300 B | +51 B | shipped (v1) | `createContext`, `provide`, `inject`, `setSsrContextMap`, `clearSsrContextMap`, `runWithContext`. Subpath: `./ssr` |
| `@scribe/signals` | 1.81 kB | 1970 B | +120 B | shipped (v0) | `signal`, `computed`, `effect`, `batch`, `untrack`, `$state`, `SignalError`, `SignalCircularError` |
| `@scribe/arbor` | 2.16 kB | 2200 B | **−15 B OVER** | shipped (v1) — **size regression** | `branch`, `leaf`, `mount`, `hydrate`, `slot`, `when`, `each`, `ArborError`, `ArborNotImplementedError` |
| `@scribe/runtime` | 1.14 kB | 1170 B | +7 B | shipped (v1) | `defineComponent`, `defineElement`, internal: `_setMount`, `_setSignal`, `_setHydrate`, `_hmrReplace`, `_hydrateOnVisible` |
| `@scribe/agent` | 117 B | 200 B | +83 B | shipped (v0) | `getAgentMetadata`, `registerAgentMetadata` |
| `@scribe/data` | 711 B | 750 B | +39 B | shipped (v1) | `createResource`, `createResourceStore`, `ResourceStoreToken`, `createResourceSerializer` |
| `@scribe/router` | 1.45 kB | 1536 B | +50 B | shipped (v1, PR #21) | `createRouter`, `viteRouterPlugin`. File-based, scans `pages/`, emits `virtual:scribe-routes` |
| `@scribe/agent-service` | 580 B | 600 B | +20 B | shipped (v1, PR #23, Plan 5.2) | `createAgentService` (yields MCP-shaped tool entries from agent registry) |
| `@scribe/server` | (no size-limit row) | — | — | shipped (v1, PR #27) | `defineRoute`, `createRouter`, `defineMiddleware`, `composeMiddleware`, `defineApiRoute`, `json`, `notFound`, `methodNotAllowed`, `badRequest`, `serverError`, `defineLoader`, `renderToString` (from `loader.ts`), `renderToStream`, `_setContextFns`, `defineScribeConfig` |
| `@scribe/agent-readiness` | (no size-limit row) | — | — | shipped (v0+v1) | `generateLlmsTxt`, `generateLlmsFullTxt`, `generateMcpServerCard`, `generateRobotsTxt`, `AI_BOT_LIST`, `createContentNegotiationHandler`, `agentReadiness` (Vite plugin), `createAgentReadinessRoutes` |
| `@scribe/compiler` | (Rust) | n/a | n/a | in-flight (Phase 1 shipped, C-3/C-4 active) | Rust crate + `bin/scribe-compile`; emits options-form `.scribe` SFCs; islands defer hydration; scoped/global styles; `<agent>` block; HMR injection via Vite plugin |

### 1.3 Notable invariants

- **Tier-3 hooks (Learning #16):** preserved in `@scribe/arbor` — subscription identity, `pathBase`, telemetry no-op-default, `Branch`/`Leaf` hidden-class shape.
- **`untrack` re-entrancy fix (Learning #46):** lattice-signal commit pattern depends on `untrack()` wrap; honored in `@scribe/data/resource.ts`.
- **`__DEV__` gate:** all 5 `_observeMount` call sites in arbor gated; rolldown `transform.define` injects `false` in production builds.
- **Hard layering boundary (per `.team/v1/spec-v1-architecture-ratified.md` §11):** `@scribe/server` and `@scribe/agent-readiness` may not be imported by browser packages; `@scribe/context` and `@scribe/data` may not import from `@scribe/server`.
...
- **v3 dep-free thesis (Learning #49):** zero non-`@scribe/*` runtime deps target. Current state below in §2 confirms this is essentially honored at runtime; deviations live in build-time tooling only.
...
- `59313b7` PR #23 — `feat/v1-agent-service` (Plan 5.2: `@scribe/agent-service` package)
- `17fc43d` PR #21 — `feat/v1-router` (Plan 6.1: `@scribe/router` file-based + Vite plugin)
...
- `@scribe/compiler`: `peerDependencies: { vite: ">=5.0.0" }` (optional)
- `@scribe/server`: `dependencies: { @scribe/agent }`; `optionalDependencies` for 4 platform-native packages (`@scribe/server-{darwin-arm64,darwin-x64,linux-x64-gnu,win32-x64-msvc}` v0.1.0)
- `@scribe/router`: `dependencies: { @scribe/server }`
- `@scribe/agent-readiness`: `dependencies: { @scribe/server, @scribe/agent }`
- `@scribe/agent-service`: `dependencies: { @scribe/agent }`
- `@scribe/data`: `dependencies: { @scribe/signals, @scribe/context }`
- `@scribe/arbor`: `dependencies: { @scribe/signals }`
- `@scribe/runtime`: `peerDependencies: { @scribe/arbor, @scribe/signals }`
- `@scribe/context`, `@scribe/signals`, `@scribe/agent`: zero `dependencies`/`peerDependencies`
- **No package declares any non-`@scribe/*` runtime dependency.** The server's optional native addons are also under the `@scribe/server-*` prefix.
...
| Capability | Nuxt baseline | Next baseline | scribe today | Status | Cite / location |
|---|---|---|---|---|---|
| Reactive primitives | (Vue) | (React) | `@scribe/signals` (signal/computed/effect/batch/untrack/$state) | shipped | `packages/signals/src/index.ts`; v0 |
| Component model | Vue SFC | React JSX | `.scribe` SFC + `@scribe/runtime` (`defineComponent`/`defineElement`) | shipped | `packages/runtime/src/index.ts`; PR #14 (compiler Phase 1), v0 |
...
| Hydration (full) | yes | `'use client'` | `serialize()` + `hydrate()` + `defineElement` hydration; `data-scribe-path` anchors | shipped | PR #25 `da83803`; `packages/arbor/src/hydrate.ts` |
...
| Router (file-based) | `pages/` | `app/` | `@scribe/router` — `createRouter`, `viteRouterPlugin` scans `pages/`, emits `virtual:scribe-routes`; static→param→catchall priority | shipped | PR #21 `17fc43d`; `packages/router/src/{router,vite-plugin}.ts` |
...
| Image optimization | `<NuxtImg>` (Sharp) | `<Image>` (Sharp) | None | **GAP** (v1.5 — Decision 3) | will become `@scribe/image` |
| Font optimization | `@nuxtjs/fontaine` | `next/font` | None | **GAP** (v1.5 — Decision 3) | will become `@scribe/fonts` |
| CSS pipeline (PostCSS, Tailwind) | yes | yes | scoped/global only via compiler; no PostCSS/Tailwind orchestration | **partial / GAP** | will become `@scribe/css-pipeline` (v1.5) |
...
| Environment vars | `runtimeConfig` | `process.env` | Not designed; `defineScribeConfig` exists but no env-var contract | **GAP** | `packages/server/src/config.ts` |
| Agent / MCP surface | (none) | (none) | `@scribe/agent` (registry) + `@scribe/agent-service` (MCP tool-shape adapter) + `@scribe/agent-readiness` (llms.txt, mcp-server-card, robots.txt, content-negotiation) — **scribe-unique** | shipped | `packages/agent*/`; v0 + Plan 5.2 + agent-readiness |
...
For every package: `dependencies` + `peerDependencies` are exclusively `@scribe/*` or absent. Scribe is **already v3-thesis compliant at runtime**. Specifically:

- No Hono/Express/Polka/Fastify/h3 in `@scribe/server` ✓
- No `path-to-regexp` in `@scribe/router` (matcher is hand-written in `router.ts:matchRoute`) ✓
- No CSS-in-JS runtime ✓
- No GraphQL client deps in `@scribe/data` ✓
...
- HMR client (PR #24) ships `_hmrReplace` in runtime (≤ ~100 B) and a Vite-plugin injection step. Whether the *injected* HMR client code at runtime is scribe-native or pulls `@vitejs/client` is not confirmed by the plan-doc — see Question Q4 in §3.
...
The Architect's Round 2 brief (per Director note Decision 6 + Decision 5 surface conditions) is to draft `2026-05-02-scribe-v1-framework.md` + companion. These questions need answers (or ratified assumptions) before the roadmap can sequence v1.1 / v1.5 / v2 / v3:

1. **Q1 — `@scribe/arbor` 15 B size regression.** Current measurement: `2.16 kB / 2200 B (15 B OVER LIMIT)`. State-plan-a.md said arbor closed at +15 B headroom after the `e94d7b1` Compressor pass; today's `bun run size` shows 15 B over. Did a later commit add bytes? Should v1.1 Builder address this before any new arbor work, or should the limit be raised per Learning #42 split (feature vs accepted debt)? Cite: `bun run size` on `7fa0957`.

2. **Q2 — Missing size-limit rows for `@scribe/server` and `@scribe/agent-readiness`.** Neither package has an entry in `.size-limit.json`. Server is by design SSR-side (Node/Bun/Workers, not browser-budgeted), but the v3 thesis still wants a measured size for the dep-free audit. Is "no row" intentional (server is dev/runtime, not browser bundle) or an oversight? Roadmap should clarify the policy explicitly.
...
4. **Q4 — HMR client: scribe-native today or `@vitejs/client`-shaped?** The `_hmrReplace` runtime export is scribe-native, but the plan-doc and PR #24 commit message describe the HMR injection mechanism as "Vite plugin injection." Is there `@vitejs/client` in the runtime call graph at dev-time, or is the entire HMR loop scribe-native (with Vite as merely the WS transport)? The v3 thesis says "no `@vitejs/client` shipped at runtime" (Learning #49). Need confirmation before the roadmap can sequence the v3 cutover.

5. **Q5 — Server-native loader: SSR-only or full edge/Workers adapter?** PR #27 server-native ships `loader.ts` with 3 states: `NATIVE_LOADED`, `EDGE_SKIPPED`, `NATIVE_FAILED_LOUD`. Edge runtime auto-skips to TS. Is this the v3 adapter pattern (auto-detect runtime; fall through to portable TS) or is a separate `@scribe/edge` / `@scribe/workers` adapter still planned? The roadmap's v2 / v3 adapter matrix depends on this.

6. **Q6 — `@scribe/router` middleware signature.** Decision 5.1 lists "Public API shape of `@scribe/router` (route-tree representation, dynamic-segment syntax, middleware signature)" as user-reserved. The router uses `static → param → catchall` priority and exposes `RouteSegment`/`RouteDefinition`/`MatchResult`. Server-side middleware is `defineMiddleware`/`composeMiddleware` in `@scribe/server`. **Is router-level middleware (separate from server-level) part of v1, v2, or undecided?** If undecided, the Architect needs to surface options.

7. **Q7 — `@scribe/data` resource graph topology (Learning #41 hazard).** Decision 5.1 lists "Public API shape of `@scribe/data` v2+ extensions (resource graph topology — same Learning #41 hazard)" as user-reserved. The current API (`createResource(key, fetcher, options?)` + store + serializer) is fetcher-shape. v2 plans `@scribe/magna` as a layer over `@scribe/data`. Does the v2 design require ANY change to the v1 `Resource<T>` shape, or is the v1 surface frozen for compat? Roadmap needs an explicit "v1 freeze line" answer.

8. **Q8 — Plugin/module system shape (`defineNuxtModule`-equivalent).** Decision 5.1 reserves this for user. The current "plugins" surface is Vite-plugin-style (`viteRouterPlugin`, `agentReadiness`, compiler plugin). Does the v2 `@scribe/plugin` surface aim to abstract over Vite (so plugins survive a v3 build-tool swap), or is it Vite-shaped permanently?
...
14. **Q14 — `@scribe/server` loud-loader policy persistence.** Director session-002 reverted `SCRIBE_FORCE_NATIVE` and made loader default loud (per `1e19da1`). Does v1.5 or v2 introduce a per-deployment opt-out that's not env-var-shaped (e.g., a config file flag), or is `SCRIBE_NATIVE_SKIP=1` the permanent contract? Roadmap should surface.

15. **Q15 — `@scribe/agent-service` Plan 5.3 wiring.** PR #23 commit message states "`handleToolCall()` (stub, Plan 5.3 wires full binding)". Plan 5.3 is not in any current build-manifest. Is 5.3 a v1.1 carry-over or a v2 item? Cite: `9 (PR #23 commit body)`.
