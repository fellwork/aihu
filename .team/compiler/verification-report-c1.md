...
| C1-2 | `pub meta: ScriptMeta` in `ScribeSource<'a>`; all 5 sfc_split snapshots include `meta` field | PASS | `types.rs` line 6: `pub meta: ScriptMeta`. All 5 snap files confirmed — `meta: ScriptMeta { name: None, }` present in all five. |
...
| C1-5 | `v-show` → `Err` with message containing `"unknown directive 'v-show'"` | PASS | `directives.rs` lines 29–37: format produces `"unknown directive 'v-show'; scribe v0 supports @event, :attr, and { identifier } only"`. Snap confirms. |
...
| C1-10 | `lib.rs` re-exports `Attr`, `CompileError`, `ScribeSource`, `ScriptMeta`, `TemplateNode` | PASS | `lib.rs` line 4: all 5 types present in re-export. |
...
**Result: PASS**
Line 8: `"unknown directive 'v-show'; scribe v0 supports @event, :attr, and { identifier } only"`. Contains required substring.
