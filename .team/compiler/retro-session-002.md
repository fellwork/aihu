...
1. **`parse_template()` not yet connected to `sfc.rs`.** The template parser exists and is tested in isolation via `tests/template_parse.rs`. The `sfc.rs::parse()` function returns `AihuSource { template: Option<&str>, ... }` but does not call `parse_template()` on that slice. The Director note for Session 2 flagged this and deferred the wiring decision (C-2 or C-3). Session 3 must resolve it — if C-2 needs the AST for signal resolution it must be wired in C-2; if signal resolution works from the raw script text only, wiring can wait until C-3.
...
- `packages/compiler/src/types.rs` — `TemplateNode`, `Attr` enums; `AihuSource` adds `pub meta: ScriptMeta`
...
- `packages/compiler/src/parser/sfc.rs` — wires `extract_script_meta` result into `AihuSource` return
...
| `extract_script_meta` was discarded (`_meta`) at `sfc.rs:128` — wired in C-1 | **NO** | Now derivable from source. After C-1, `sfc.rs` stores the result in `AihuSource`. Historical "it was discarded" fact has no future relevance. |
