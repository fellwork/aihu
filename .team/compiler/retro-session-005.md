...
**Codex sandbox boundary prevents direct worktree writes.** Codex cannot write to worktree paths — the sandbox root is the main checkout. The Builder pass was implemented directly by Team Lead rather than Codex. This is a known constraint; the fix is to always use the Codex worktree flag, which was used correctly here (worktree `aihu-compiler-c3` is the Codex target).

**Windows `.exe` binary path was missed in the initial Builder pass.** The `js/index.ts` binary resolution used `../target/release/aihu-compile` which silently fails on Windows (binary is `aihu-compile.exe`). The fix landed in a follow-up commit (a0af4d4). Pre-merge checklist should include: "verify binary path includes platform extension detection."
...
| `bun vite build` does not invoke Vite plugin transform hooks when Bun is the config loader (Bun/Rollup4 ESM incompatibility) | **YES** | Non-obvious. Any `@aihu/compiler` Vite integration test must call `transform()` directly or run via Node/Vite programmatic API, not `bun vite build`. |
...
| `enforce: 'pre'` is required for any Vite plugin that transforms custom file types | **YES** | Without it, other plugins may process `.aihu` files first. Correct plugin shape: `{ name, enforce: 'pre', transform(code, id) {} }`. |
...
| C-4 | CLI binary + @aihu/compiler npm package | COMPLETE |
...
5. Investigate and document the `bun vite build` / Bun+Rollup4 ESM plugin incompatibility for the `@aihu/compiler` package.
