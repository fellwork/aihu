...
**Date:** 2026-04-30
**Track:** plan-a
**Branch:** claude/scribe-phase-3-team-Za4UQ (post-merge)
...
- `mount-10k-leaves`: scribe 36.95 ms vs lit-html 5.65 s vs preact 66.01 ms vs vanilla 92.60 ms — scribe fastest
- `mount-deep-100x10`: scribe 3.44 ms vs preact 9.35 ms vs lit-html 61.88 ms — scribe fastest
- `mount-wide-1000`: scribe 8.57 ms vs preact 10.48 ms vs vanilla 12.98 ms — scribe fastest
- `update-1-of-10k-leaves`: scribe **25 ns** (p50) vs vanilla 3.1 µs — **122× faster** (nodeValue vs textContent)
- `krausest-1k-cycle`: scribe 20.9 ms vs preact 19.7 ms (near-tie) vs vanilla 16.1 ms — acceptable overhead

**arbor notes:**
- solid-js and @vue/runtime-dom ERROR in all workloads (JSDOM env gaps — deferred Round N+2)
- `attr-thrash-100x100`: lit-html, solid, vue all ERROR in JSDOM; only preact/vanilla/scribe run (inconclusive axis)

**signals wins:**
- `cellx` (5-deep diamond): scribe 506 ns p50 vs alien 675 ns — scribe wins
- `batched-writes-100`: scribe 2.60 µs vs alien 3.54 µs — scribe wins
- `dynamic-deps` (rotating fan-in/fan-out): scribe 742 ns vs alien 1.21 µs — scribe wins 1.6×
- `creation-1to1000` (solid parity): scribe 69.3 µs vs alien 91.1 µs — scribe wins

**signals losses (honest):**
- `deep-propagation-100` (100-deep chain): scribe 4.0 µs vs alien 2.4 µs — **1.65× slower** (deep-chain gap)
- `wide-fanout-100`: scribe 4.68 µs vs alien 3.29 µs — alien faster on pure fan-out
...
**Arbor:** YES — Round N+1 satisfies Learning #11 for arbor. Scribe is fastest on all 4 mount/update workloads vs the JSDOM-compatible competitors (lit-html, preact, vanilla). The `update-1-of-10k-leaves` 122× win is the headline axis. The krausest near-tie with preact is acceptable overhead. solid-js/vue comparison is deferred to a browser runner (Round N+2) — that's an environment limitation, not a performance limitation.

**Signals:** YES — Round N+1 satisfies Learning #11 for signals. Prior work (Phase 2.5) established time receipts. Round N+1 adds memory receipts and closes the parity workload gap. Scribe wins on cellx, batched-writes, dynamic-deps, and creation-1to1000. The deep-propagation loss (1.65×) is documented honestly (Learning #26) and assigned to v0+1 work — scribe is tuned for shallow diamonds, and that is a known, documented design point, not a surprise.
...
- bench/arbor/tsconfig.json shipped without `paths` entries for `@scribe/arbor` and `@scribe/signals` — typecheck failed on the team branch until the Historian fixed it (needed `dist/` path mappings, not `src/` to avoid rootDir violations)
...
`element.textContent = v` is a multi-step DOM operation (child list walk, remove, recreate text nodes). `textNode.nodeValue = v` is a direct property set. In JSDOM, the gap is 122× (3.1 µs vs 25 ns). Arbor's `leaf(signal)` uses `nodeValue` internally via `materialize.ts`. This is the source of scribe's headline `update-1-of-10k-leaves` advantage — not just signal efficiency, but the bind target.
...
`deep-propagation-100` (100-deep linear chain): scribe 4.0 µs vs alien-signals 2.4 µs (1.65× slower). `dynamic-deps` (rotating fan-in/fan-out): scribe 742 ns vs alien 1.21 µs (scribe wins 1.6×). The pattern: scribe's forward-subscription model dominates on dynamic-dependency changes but loses on pure deep-chain propagation. The cellx win is a shallow-diamond shape; alien's deep-chain tuning is the gap to close in v0+1 signals work.

**#27 — krausest-in-JSDOM: vanilla wins on 1k-cycle because it skips signals entirely**

Vanilla DOM in `krausest-1k-cycle` builds a 1k-row table with direct createElement + textContent (no reactive layer). Scribe adds a signals reactive layer with 1 signal per cell (2 cells/row × 1k rows = 2k signals). The ~30% scribe-vs-vanilla overhead is the reactive bookkeeping cost for a pre-mounted tree. This is acceptable overhead for a system that also wins 122× on targeted reactive updates (`update-1-of-10k-leaves`). The two numbers together define scribe's design point: pay a modest mount overhead, win hugely on update.
