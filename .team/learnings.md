...
## 38. Rust compiler layers: parse-phase (grammar routing) and analysis-phase (semantic validation) are distinct and should be planned as separate milestones

**Why.** v0.2.2 delivered `Grammar::At` body extraction — detecting `@blockname {`, extracting the body, mapping to the correct `BlockKind`. This was a complete parse-phase implementation. When v0.3 planning listed "8 sub-items migrating the parser stub to a full dual-grammar parser + emitter," the reality was that the parse phase was already done; the 8 sub-items were analysis-phase work (`$global` scope, deprecation warnings, undeclared ref warnings, reserved-token rejection). The Builder completed all 8 in 2 commits, well under the planned budget, because the parse-phase infrastructure was solid.

**How to apply.** When planning Rust compiler milestones, explicitly name the layer each sub-item targets: (1) **parse phase** — grammar routing, body extraction, block kind mapping; (2) **analysis phase** — semantic validation, warnings, error codes; (3) **emit phase** — JS lowering, code generation. A sub-item that is "analysis" can be parallelised freely once parse is solid; a sub-item that is "emit" depends on both parse and analysis being complete. If a parser stub already covers parse phase, the next milestone's sub-items are analysis + emit work — budget them accordingly (analysis is fast once parse is solid; emit is the bulk of effort).
