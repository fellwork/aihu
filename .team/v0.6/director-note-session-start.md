...
- v0.6.1: @route block parser + RouteBlock struct + ScribeSource.route field
...
- v0.6.3: @scribe/router vite-plugin scanPages() reads .route.json sidecars
- v0.6.5: build.target field in defineScribeConfig (ScribeConfig extension)
- v0.6.7: $server macro lowering + createServerCall helper in @scribe/server client subpath
- v0.6.8: file-based layouts (layouts/default.scribe scan + virtual:scribe-layouts)
...
```
Stream B's RouteDefinition in @scribe/router should add nullable fields for these.
...
- **createServerCall** goes in @scribe/server client subpath (NOT @scribe/runtime)
- **Router size**: @scribe/router stays ≤1536 B (route metadata consumption is build-time only, tree-shaked from browser bundle)
- **No new runtime dep** — v3 dep-free thesis must hold
- **Acceptance gate**: `src/pages/admin/users.scribe` with `@route { name: 'admin-users', layout: 'admin', middleware: ['auth'] }` compiles → sidecar emitted → router scanPages() reads it; `--target client` elides server artifacts

## Surface conditions

1. Any new non-@scribe/* runtime dep → surface
2. @scribe/router browser bundle grows beyond 1536 B → surface (router metadata is build-time only)
