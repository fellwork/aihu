# Investigation — `@aihu/data` Externalization Bug
...
every value-import from `@aihu/signals` (and `@aihu/context`) pulls the
...
`external: ['@aihu/signals', '@aihu/context']` to data's rolldown config
...
"@aihu/signals"; import {createContext, inject} from "@aihu/context";`
...
data:build`, `bun run --filter @aihu/data build`, and `cd packages/data &&
...
$ bun run size 2>&1 | grep data
  ✗ @aihu/data      2.00 kB /   750 B  (1298 B OVER LIMIT)
```

Dist head shows two `class extends Error` definitions named `SignalError` and
`SignalCircularError` — these are private classes from `@aihu/signals/src/`,
...
```
$ rm -rf packages/data/dist
$ bun run --filter @aihu/data build
@aihu/data build: <DIR>/index.js  chunk │ size: 4.95 kB
$ ls -la packages/data/dist/index.js
-rw-r--r-- ... 4951 ... packages/data/dist/index.js
$ head -c 200 packages/data/dist/index.js
...
| Build path | Raw bytes | Gzip bytes | `bun run size` | Inlines `@aihu/signals`? |
|---|---|---|---|---|
| `bunx moon run data:build` | 4951 | 2012 | 2.00 kB / 750 B (1298 B OVER) | YES |
| `bun run --filter @aihu/data build` | 4951 | 2012 | 2.00 kB / 750 B (1298 B OVER) | YES |
...
```
$ # rolldown.config.ts modified to add: external: ['@aihu/signals', '@aihu/context']
$ rm -rf packages/data/dist && bunx moon run data:build
$ head -c 200 packages/data/dist/index.js
import{batch as e,boolLatticeSignal as t,effect as n,maxLatticeSignal as r,signal as i,untrack as a}from"@aihu/signals";import{createContext as o,inject as s}from"@aihu/context";function c()...
$ gzip -c packages/data/dist/index.js | wc -c
764
$ bun run size 2>&1 | grep data
  ✓ @aihu/data        747 B /   750 B  (+3 B headroom)
```
...
| H2 | Rolldown config missing `external` for `@aihu/data` | Add `external: ['@aihu/signals', '@aihu/context']` to `packages/data/rolldown.config.ts`; rebuild | **CONFIRMED.** §5 shows dist switches from inlined to `import{...}from"@aihu/signals"`; gzip 2012 → 764 B; size script PASS at 747 B. Reverting re-introduces inlining. |
| H3 | `package.json` peer-dep gap (`@aihu/signals` in `dependencies`, not `peerDependencies`) | Move both deps to `peerDependencies` WITHOUT adding rolldown `external`; rebuild | **FALSIFIED.** Dist still inlines (head: `var e=class extends Error{}...SignalError...`); gzip remains 2012 B. Rolldown does NOT auto-externalize peerDeps from package.json — they must be in the rolldown `external` config. |
...
| H5 | Sister-package config diff suggests something else | Check whether other packages that import `@aihu/signals` correctly externalize | **PARTIALLY INFORMATIVE; SUPPORTS H2.** §3 walk: `@aihu/runtime` declares `external: ['@aihu/arbor', '@aihu/signals']` and externalizes (no value-imports of those packages anyway, so external is defense-in-depth). `@aihu/arbor` declares NO `external` and INLINES `@aihu/signals` (just like data); only fits its 2200 B limit because its limit is loose. `@aihu/agent` has no workspace deps. Pattern: the only package that successfully externalizes a workspace dep it actually value-imports is the one with explicit `external` declared. |
| H6 (added) | Rolldown auto-externalizes some shapes (`type-only` imports, `import type`) but inlines value imports | Examine which symbols `@aihu/data/src/*` actually value-imports | Confirms H2 framing. data value-imports `signal`, `batch`, `boolLatticeSignal`, `maxLatticeSignal`, `untrack`, `effect`, `createContext`, `inject`. None type-only. With no `external`, rolldown follows every value import. |
...
- **package.json:** `peerDependencies: { @aihu/arbor, @aihu/signals }`. No `dependencies`.
- **rolldown.config.ts:** `external: ['@aihu/arbor', '@aihu/signals']` declared explicitly.
- **src value imports of workspace deps:** none. `runtime/src/types.ts`, `define-component.ts` only `import type` from `@aihu/arbor` and `@aihu/signals` (verified via `grep "from ['\"]@aihu" packages/runtime/src`). `import type` is erased by tsc, so even WITHOUT `external` the dist would have nothing to inline.
...
- **package.json:** `dependencies: { @aihu/signals: workspace:* }`.
- **rolldown.config.ts:** **NO `external` declaration.** Identical shape to data's config.
- **src value imports:** `arbor/src/mount.ts` line 1 imports `effect` (a value) from `@aihu/signals`. Five other arbor files `import type { Dispose, Signal }` (type-only).
...
- **`bun run size` reports** `@aihu/arbor 2.10 kB / 2200 B (+47 B headroom)` — passes only because the limit is loose enough to fit inlined signals + its own code + (the size script's secondary pass which has nothing to externalize either, since arbor's `.size-limit.json` entry has no `ignore` field).
...
|---|---|---|---|---|
| `@aihu/runtime` | NO (type-only) | YES | N/A (nothing to inline) | NO |
| `@aihu/arbor` | YES (`effect`) | NO | **YES** | NO (limit 2200 B accommodates) |
| `@aihu/data` | YES (8 symbols) | NO | **YES** | **YES** (limit 750 B does not accommodate) |
| `@aihu/agent` | NO | NO | N/A | NO |
| `@aihu/context` | NO | NO | N/A | NO |

**Bug status: NOT local to `@aihu/data`. It is a repo-wide pattern that
manifests as a size-gate failure only on `@aihu/data` because data is the
only package with both (a) a value-import of a workspace dep AND (b) a tight
enough limit to expose the inlining.** `@aihu/arbor` has the same defect
silently.
...
- **Narrow (Director-brief default):** add `external` to
  ...
  shipped to consumers under the `@aihu/arbor` package).
- **Wide:** add `external` to both data and arbor configs. Closes the dormant
  defect. May change `@aihu/arbor`'s gzip size measurably; would need to
  re-check its limit. **This expands scope per Decision 2 trigger #14
  (Builder edits files outside `@aihu/data` package + `.size-limit.json` +
...
workspace deps that data value-imports (`@aihu/signals`, `@aihu/context`).
Once asserted, rolldown stops following value imports into those graphs and
emits `import { ... } from '@aihu/signals'` (and similarly for context) at
...
- H5 (sister-package config) corroborates: `@aihu/runtime` is the only
  workspace-dep-importing package with `external` declared, and the only one
  whose dist is correctly externalized. `@aihu/arbor` has the same gap and
  the same inlining behavior.

### Citation

- **File to change:** `packages/data/rolldown.config.ts`
- **Line to change:** line 7 (insert `external: ['@aihu/signals', '@aihu/context'],` after the existing `checks:` line; or any other syntactic position that puts the property on the input config object).
...
- **`@aihu/arbor` has the same defect.** Closing it changes scope; surface
...
- **`package.json` `dependencies` shape is fine.** `@aihu/signals` and
  `@aihu/context` belong as `dependencies` (not `peerDependencies`) for
...
- **`.size-limit.json` is correct.** The `ignore: ['@aihu/signals',
  '@aihu/context']` field is for the size script's own bundling pass over
...
**Edit (NOT committed):** `packages/data/rolldown.config.ts`, add `external: ['@aihu/signals', '@aihu/context'],` after `checks: { circularDependency: true },`.
...
```
import{batch as e,boolLatticeSignal as t,effect as n,maxLatticeSignal as r,signal as i,untrack as a}from"@aihu/signals";import{createContext as o,inject as s}from"@aihu/context";function c(){let e=new Map,t=new Set;return{get:t=>e.get(t),set:(t,n)=>{...
```

Both upstream packages are now `import { ... } from "@aihu/<pkg>"`
...
- `packages/arbor/src/mount.ts` line 1 (only value-import of `@aihu/signals` from arbor)
- `.size-limit.json` lines 32-38 (`@aihu/data` entry; `ignore` field; canonical 750 B limit confirmed on `f53afff`)
