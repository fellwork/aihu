...
Add `onMount` + `onCleanup` lifecycle helper exports to `@scribe/runtime`. Pre-authorized Compressor pass (Polish Note 5) to recover bytes before adding new exports. Runtime is at +7 B headroom (1.14 kB / 1170 B); adding ~30-50 B requires ~23-43 B Compressor recovery minimum.
...
- **Dep envelope unchanged** — zero non-`@scribe/*` runtime deps.
...
1. **Compressor pass on `@scribe/runtime`** (Polish Note 5): auto-proceeds without re-surfacing. Only fires a surface signal if recovery falls short of new feature cost AND a limit raise is needed (Learning #42 split formal raise).
...
3. Any new TS dep outside `@scribe/*` → surface, reject
