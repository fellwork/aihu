...
End-of-Phase-4 exports from `@aihu/runtime` (re-exported through `packages/runtime/src/index.ts`):
...
```js
defineElement('hello-aihu', HelloAihu)
```

where `HelloAihu` is a `class HelloAihu extends HTMLElement` the compiler already fully defined, with `connectedCallback`, `disconnectedCallback`, `static observedAttributes`, `attributeChangedCallback`, and a `_build()` method producing the arbor tree.
...
`defineElement` does not call `mount()`. The compiler-emitted `connectedCallback` calls `mount()`. Runtime has zero import-time dependency on `@aihu/arbor`.
...
> **Phase 4 session note (added 2026-04-26 by Team Lead, post-Phase-3 spec session).** The Phase 3 spec-arbor session locked a **two-layer authoring model** (Learning #12): `defineElement(name, Ctor)` is for compiler-emitted code; `defineComponent(setup)` is the functional helper for hand-authored components. **`defineComponent(setup)` lives in `@aihu/runtime`** and is added in Phase 4 alongside `defineElement`.
...
```ts
import type { Branch, Leaf } from '@aihu/arbor'
...
```ts
/** @internal */
const SHADOW_ROOT_SYM = Symbol('aihu.shadowRoot')
...
```
@aihu/runtime source
  ↳ no imports from @aihu/arbor
  ↳ no imports from @aihu/signals
  ↳ browser DOM API only (HTMLElement, ShadowRoot, customElements)

@aihu/arbor  — peerDependency (type resolution for consumers, not bundled)
@aihu/signals — peerDependency (same reason)
```
...
Per Team Lead Call 1, `untrack` exists in `@aihu/signals` as of Phase 3 prep. `wrapClass` and `defineElement` have no reactive reads — `attachShadow` is a plain DOM call in a constructor. No `untrack` import in runtime source.
...
`external` is required here — unlike signals and arbor (which have no cross-package imports), runtime's peer deps must not be accidentally bundled. Without `external`, rolldown would attempt to resolve `@aihu/arbor` at build time and fail (no source-level import exists in runtime, so this is precautionary).

### 3.5 `.size-limit.json` — add `@aihu/runtime` row (Task 21)
...
- `bun run size` exits 0 (`@aihu/runtime` row not added yet; size-limit only errors when a row's path file is missing after the row exists).
...
These are the first tests to exercise `@aihu/runtime` + `@aihu/arbor` + `@aihu/signals` together. They require `@aihu/arbor` to be shipped (Phase 3 complete). **Task 22 is blocked on Phase 3.** If phases run in sequence, Task 22 follows Phase 3's completion. If phases partially overlap, the unit tests (Task 21) can ship without arbor; Task 22 waits.
...
Commit: `feat(runtime): scaffold @aihu/runtime package`
...
| `.size-limit.json` | modify | Add `@aihu/runtime` row at 1024 B |
...
| 6 | Zero source-level imports from `@aihu/arbor` or `@aihu/signals` in runtime | Decision 2B | `defineElement` does not call `mount()`. This structural choice is what makes the 1 KB budget achievable. Peer deps declared for type resolution only. |
| 7 | `rolldown.config.ts` adds `external: ['@aihu/arbor', '@aihu/signals']` | Decision 2B | Prevents accidental bundling of peer deps. No precedent in signals config (signals has no cross-package deps); explicit here. |
...
| 9 | Task 22 (integration) is blocked on Phase 3 (`@aihu/arbor`) | Decision 2B | Tasks 20–21 are self-contained; Task 22 requires arbor to be shipped. Builder must not start Task 22 until arbor is available. |

---

## 7. Open questions for the Team Lead

Assumptions about `@aihu/arbor`'s public surface that Architect A's `spec-arbor.md` may invalidate. Team Lead must reconcile after both specs land.
...
Integration tests import `branch`, `leaf` from `@aihu/arbor`. The value imports are enough for JSDOM tests; type imports of `Branch`/`Leaf` are used in handwritten test component signatures. If Architect A does not export the types, test code uses `ReturnType<typeof branch>` instead — no blocker, but inelegant.
...
- [x] Size budget row for @aihu/runtime specified in §3.5 at 1024 B gzip.
-->
