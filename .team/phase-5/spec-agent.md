...
End-of-Phase-5 exports from `@scribe/agent` (re-exported through `packages/agent/src/index.ts`):
...
**Magna alignment.** Where `<agent>` blocks describe what *components* expose (state shape + actions), magna's GraphQL introspection describes what the *backend* exposes (queries, mutations, types). A unified capability manifest can include both surfaces — agents loading the manifest see the complete picture: "this app exposes these reactive components, backed by this data schema." Magna ships introspection out of the box; scribe's `<agent>` registry is the per-component complement.
...
The compiler emits, at the top level of each `.scribe`-derived JS module:
...
- **Explicit named import** — the compiler knows the exact function name at emit time; if `@scribe/agent` changes its export shape, the import fails with a clear TypeScript error at the consuming module's type-check, not silently at runtime.
...
- **Last-registration-wins for HMR** — when a `.scribe` file's `<agent>` block changes and the HMR runtime re-executes the module, `registerAgentMetadata` is called again with the new frozen object. The registry overwrites the old entry by tag. No teardown needed; no stale entry survives. This matches the spec §10.3 framing: "HMR swaps one component's class; state survival is best-effort." The registry update is atomic (Map.set), not best-effort.
- **No side-effect-only import** — a bare `import '@scribe/agent/register'` side-effect module was considered and rejected: it would require a second `@scribe/agent` entry point, complicating the package's exports map and the size-limit path.
...
One line. Inherits `build` and `typecheck` from `.moon/tasks.yml` (same pattern as `@scribe/signals`). No custom inputs or outputs — agent has no bench tasks, no jsdom environment, no special rolldown config.
...
Rationale: the package is a `Map` plus two functions and one interface. The registry `Map` initialization, two function literals, and the `export` wrappers compress to well under 100 B gz — the Phase 2 analogy is `@scribe/signals`'s empty scaffold starting at 108 B raw before any logic. A package with two functions over a single `Map` will be smaller still. Setting 100 B forces CI to catch any inadvertent dependency pull (e.g. accidentally importing `@scribe/signals`) immediately. If the implementation lands at 80 B gz, 100 B gives 25% headroom for future additions within v0 (e.g. a `listAgentTags()` utility). If for some reason the bundled output exceeds 100 B gz, the Builder raises it to the Team Lead with measurement evidence before adjusting the gate — not silently bumping the limit.
...
The root `vitest.config.ts` already declares an `@scribe/agent` alias (plan line ~366):
...
| 2 | Registered tag returns the metadata object | `getAgentMetadata('hello-scribe')` returns the registered object |
...
| File | Action | Purpose |
|---|---|---|
| **`packages/agent/package.json`** | create | `@scribe/agent` package manifest; `"name": "@scribe/agent"`, no runtime deps |
| **`packages/agent/tsconfig.json`** | create | Extends `../../tsconfig.base.json`; `include: ["src"]` |
| **`packages/agent/rolldown.config.ts`** | create | ESM + dts emit; same pattern as `@scribe/signals` |
...
| `vitest.config.ts` | verify | Confirm `@scribe/agent` alias exists; no edit if present |
