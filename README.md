...
|---|---|---|
| [`@aihu-plugin/agent-readiness`](./packages/plugin-agent-readiness) | `2.2.3` | Discovery + readiness manifest emitter so agents can introspect aihu apps. |
| [`@aihu-plugin/data`](./packages/plugin-data) | `2.0.5` | Reactive data loaders and resource primitives for aihu. |
| [`@aihu-plugin/drizzle`](./packages/plugin-drizzle) | `0.1.5` | Drizzle ORM data adapter for aihu — typed createResource fetchers and defineLoader helpers (Postgres / SQLite / libSQL). |
...
| [`@aihu/adapter-cloudflare`](./packages/adapter-cloudflare) | `12.0.0` | Cloudflare Workers/Pages deployment adapter for @aihu/app. |
| [`@aihu/adapter-vercel`](./packages/adapter-vercel) | `12.0.0` | Vercel deployment adapter for @aihu/app. |
...
| [`@aihu/agent-readiness`](./packages/_moved/agent-readiness) | `2.0.3` | [MOVED] This package has moved to @aihu-plugin/agent-readiness. |
| [`@aihu/agent-server`](./packages/agent-server) | `0.4.3` | Server-side glue: mount an aihu component server-side and let an MCP client drive it through the agent-service live-dispatch gate, forwarding approved invocations to a browser bridge. |
...
| [`@aihu/app`](./packages/app) | `9.0.0` | Top-level app integration — wires runtime, router, and adapters into a Vite app. |
| [`@aihu/arbor`](./packages/arbor) | `4.1.0` | Reactive component tree (the rendering layer that consumes @aihu/signals). |
...
| [`@aihu/compiler`](./packages/compiler) | `1.2.2` | Single File Component (.aihu) compiler — Rust binary + JS glue. |
| [`@aihu/compiler-native-darwin-arm64`](./packages/compiler/npm-native/darwin-arm64) | `0.1.18` | aihu compiler native addon (napi) — darwin-arm64 platform binary. |
| [`@aihu/compiler-native-darwin-x64`](./packages/compiler/npm-native/darwin-x64) | `0.1.18` | aihu compiler native addon (napi) — darwin-x64 platform binary. |
| [`@aihu/compiler-native-linux-arm64-gnu`](./packages/compiler/npm-native/linux-arm64-gnu) | `0.1.18` | aihu compiler native addon (napi) — linux-arm64-gnu platform binary. |
| [`@aihu/compiler-native-linux-x64-gnu`](./packages/compiler/npm-native/linux-x64-gnu) | `0.1.18` | aihu compiler native addon (napi) — linux-x64-gnu platform binary. |
| [`@aihu/compiler-native-win32-x64-msvc`](./packages/compiler/npm-native/win32-x64-msvc) | `0.1.18` | aihu compiler native addon (napi) — win32-x64-msvc platform binary. |
...
| [`@aihu/language-server`](./packages/language-server) | `0.4.0` | Cross-editor Language Server (aihu-language-server) for .aihu Single File Components — diagnostics, hover, completion, and quick-fix code actions. |
...
| [`@aihu/primitives`](./packages/primitives) | `0.2.2` | aihu headless behavior primitives — WAI-ARIA APG patterns as vanilla custom elements, zero CSS. |
...
| [`@aihu/router`](./packages/router) | `0.4.4` | File-based router for the aihu meta-framework. |
| [`@aihu/runtime`](./packages/runtime) | `6.0.0` | Single File Component (.aihu) runtime — registers custom elements compiled by @aihu/compiler. |
...
| [`@aihu/seo`](./packages/seo) | `1.0.4` | DEPRECATED compatibility shim over @aihu-plugin/agent-readiness (sitemap.xml, robots.txt, llms.txt, JSON-LD). |
| [`@aihu/server`](./packages/server) | `0.5.0` | Server runtime + native renderer (napi-rs) for aihu SSR. |
...
| [`@aihu/tsc`](./packages/tsc) | `0.3.2` | aihu-tsc — `tsc` for projects containing .aihu Single File Components. Type-checks .aihu sources as virtual TypeScript, with no .aihu.ts files written to disk. |
...
| [`@aihu/use`](./packages/use) | `1.0.0` | aihu utility/sensor/state composables — SSR-safe, scope-aware, per-composable subpath entries. |
