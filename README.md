...
| `@aihu/runtime` | 3.99 kB | 4100 B | pass |
...
| `@aihu/agent-service` | 2.49 kB | 2600 B | pass |
...
| [`@aihu/app`](./packages/app) | `4.0.0` | Top-level app integration — wires runtime, router, and adapters into a Vite app. |
| [`@aihu/arbor`](./packages/arbor) | `2.0.0` | Reactive component tree (the rendering layer that consumes @aihu/signals). |
| [`@aihu/auth`](./packages/auth) | `2.0.0` | JWT scope checks, ScopeSignal, and server middleware for aihu auth. |
| [`@aihu/cli`](./packages/cli) | `0.8.3` | Aihu CLI (`aihu`, `create-aihu`) — scaffolding, dev, build commands. |
| [`@aihu/compiler`](./packages/compiler) | `0.11.0` | Single File Component (.aihu) compiler — Rust binary + JS glue. |
| [`@aihu/context`](./packages/context) | `0.2.0` | Async-context-friendly request/SSR context primitives for aihu. |
| [`@aihu/css-engine`](./packages/css-engine) | `0.4.5` | aihu CSS engine — Tailwind v4 hard fork with WC-native scoped output. |
| [`@aihu/data`](./packages/_moved/data) | `2.0.2` | [MOVED] This package has moved to @aihu-plugin/data. |
| [`@aihu/language-server`](./packages/language-server) | `0.2.22` | Cross-editor Language Server (aihu-language-server) for .aihu Single File Components — diagnostics, hover, completion, and quick-fix code actions. |
| [`@aihu/magna`](./packages/magna) | `0.2.2` | aihu bridge for Magna GraphQL — dep-free fetch, resource composition, JWT relay |
...
| [`@aihu/primitives`](./packages/primitives) | `0.1.2` | aihu headless behavior primitives — WAI-ARIA APG patterns as vanilla custom elements, zero CSS. |
...
| [`@aihu/tsc`](./packages/tsc) | `0.2.3` | aihu-tsc — `tsc` for projects containing .aihu Single File Components. Type-checks .aihu sources as virtual TypeScript, with no .aihu.ts files written to disk. |
