...
| `@aihu/arbor` | 2.68 kB | 2800 B | pass |
...
| `@aihu/primitives/tooltip` | 1.53 kB | 4 KB | pass |
...
| [`@aihu/adapter-cloudflare`](./packages/adapter-cloudflare) | `4.0.2` | Cloudflare Workers/Pages deployment adapter for @aihu/app. |
| [`@aihu/adapter-vercel`](./packages/adapter-vercel) | `4.0.2` | Vercel deployment adapter for @aihu/app. |
...
| [`@aihu/agent-server`](./packages/agent-server) | `0.3.0` | Server-side glue: mount an aihu component server-side and let an MCP client drive it through the agent-service live-dispatch gate, forwarding approved invocations to a browser bridge. |
...
| [`@aihu/app`](./packages/app) | `3.0.2` | Top-level app integration — wires runtime, router, and adapters into a Vite app. |
...
| [`@aihu/compiler`](./packages/compiler) | `0.10.2` | Single File Component (.aihu) compiler — Rust binary + JS glue. |
...
| [`@aihu/language-server`](./packages/language-server) | `0.2.21` | Cross-editor Language Server (aihu-language-server) for .aihu Single File Components — diagnostics, hover, completion, and quick-fix code actions. |
...
| [`@aihu/primitives`](./packages/primitives) | `0.1.1` | aihu headless behavior primitives — WAI-ARIA APG patterns as vanilla custom elements, zero CSS. |
...
| [`@aihu/tsc`](./packages/tsc) | `0.2.2` | aihu-tsc — `tsc` for projects containing .aihu Single File Components. Type-checks .aihu sources as virtual TypeScript, with no .aihu.ts files written to disk. |
